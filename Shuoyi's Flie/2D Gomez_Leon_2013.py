...
custom_root_directory = "Shuoyi's Flie/data_Gomez_Leon_2013_2D"  # Replace with your desired path
...
# =============================
def track_valence_band(kx_values, ky_values, E0_over_omega, previous_val = None, previous_con = None, v = 0.15, w = 0.2, nt=61, filename=None, b=0.5, t=1.5):
    """
    ...
    occupied_states = np.zeros((Norbs*nt, len(kx_values)), dtype=complex)
    conduction_states = np.zeros((Norbs*nt, len(kx_values)), dtype=complex)
    occupied_states_energy = np.zeros(len(kx_values), len(ky_values))
    conduction_states_energy = np.zeros(len(kx_values), len(ky_values))
    ...
    if E0_over_omega == 0:
        for i in range(len(kx_values)):
            for j in range(len(ky_values)):
                k0x = kx_values[i]
                k0y = ky_values[i]
                H_0 = np.array([[0, t*jv(0,E0_over_omega*b)+np.exp(-1j*k0x)*jv(0,E0_over_omega*(1-b)), t*jv(0,E0_over_omega*b)+np.exp(-1j*k0y)*jv(0,E0_over_omega*(1-b)),0],
                                [t*jv(0,E0_over_omega*b)+np.exp(1j*k0x)*jv(0,E0_over_omega*(1-b)), 0, 0,t*jv(0,E0_over_omega*b)+np.exp(1j*k0y)*jv(0,E0_over_omega*(1-b))],
                                [t*jv(0,E0_over_omega*b)+np.exp(1j*k0y)*jv(0,E0_over_omega*(1-b)), 0, 0,t*jv(0,E0_over_omega*b)+np.exp(1j*k0x)*jv(0,E0_over_omega*(1-b))],
                                [0, t*jv(0,E0_over_omega*b)+np.exp(-1j*k0y)*jv(0,E0_over_omega*(1-b)), t*jv(0,E0_over_omega*b)+np.exp(-1j*k0x)*jv(0,E0_over_omega*(1-b)), 0]], dtype=complex)
                ...
                occ_state, occ_state_energy = floquet.winding_number_Peierls_GL2013_2D(k0x, k0y, quasi_E=None, previous_state=previous_val[:,i], w=w, b=b, t=t, E_over_omega=E0_over_omega)
                con_state, con_state_energy = floquet.winding_number_Peierls_GL2013_2D(k0x, k0y, quasi_E=None, previous_state=previous_con[:,i], w=w, b=b, t=t, E_over_omega=E0_over_omega)
                occupied_states[:,i*Norbs+j] = occ_state
                conduction_states[:,i*Norbs+j] = con_state
                occupied_states_energy[i,j] = occ_state_energy
                conduction_states_energy[i,j] = con_state_energy
    else:
        for i in range(len(kx_values)):
            for j in range(len(ky_values)):
                k0x = kx_values[i]
                ...
                if occ_state_energy < con_state_energy:
                    ...
                    occupied_states[:,i*Norbs+j] = con_state
                    conduction_states[:,i*Norbs+j] = occ_state
                    occupied_states_energy[i,j] = con_state_energy
                    conduction_states_energy[i,j] = occ_state_energy
...
def figure(occ_state_energy, cond_state_energy, kx_values):
    save_folder = "Shuoyi's Flie/Floquet_Band_Plots_Gomez_Leon_2013_2D"
    os.makedirs(save_folder, exist_ok=True)
    plt.figure(figsize=(8, 6))
    plt.plot(kx_values, occ_state_energy, label=f'Val_E0_over_omega = {E0_over_omega:.6f}, b = {b:.2f},t = {t:.2f}')
    plt.plot(kx_values, cond_state_energy, label=f'Con_E0_over_omega = {E0_over_omega:.6f}, b = {b:.2f},t = {t:.2f}')
...
for j, b in enumerate(b_values):
    ...
    occ_states, occ_state_energy, con_states, con_state_energy, draw = track_valence_band(kx_values, ky_values, E0_over_omega, v=v, w=w, nt=nt, filename=data_filename, b=b, t=t)
    if draw == True:
    ...
    for i in range(len(E0_over_omega_values)-1):
        ...
        occ_states, occ_state_energy, con_states, con_state_energy, draw = track_valence_band(kx_values,ky_values, E0_over_omega, previous_val = pre_occ, previous_con = pre_con, v=v, w=w, nt=nt, filename=data_filename, b=b, t=t)
        if draw == True:
            figure(occ_state_energy, con_state_energy, kx_values)
        W_berry_real = berry_phase_winding(kx_values, occ_states)
