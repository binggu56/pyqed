...
def test_hubbard_ladder_compiled_policy_uses_full_safe_layout():
    ...
    without_cpp.pop("moving_environment_cpp_owner_half_sweep_step_records")
    without_cpp.pop("moving_environment_cpp_owner_half_sweep_typed_records")
    without_cpp.pop("moving_environment_operatorless_local_problem")
...
def test_moving_environment_builds_and_reuses_cpp_grouped_renormalized_table():
    ...
    if getattr(cpp_davidson, "RawPayloadBuilder", None) is not None:
        ...
        assert builder_reused.last_refresh_kind in {
            "raw_dense_in_place",
        ...
        builder_env._sync_cpp_moving_environment_stats()
        ...
        assert builder_stats["cpp_grouped_renormalized_table_fast_refreshes"] >= 1
        if builder_reused.last_refresh_kind == "raw_dense_schedule_in_place":
            assert builder_reused.cpp_table.raw_schedule_refresh_hits() >= 1
...
def test_moving_environment_cpp_owner_provides_sweep_cursor():
    ...
    cpp_local_env = MovingEnvironment(
        matvec_options={"moving_environment_cpp_state_owner": True}
    )
    cpp_local_owner = getattr(cpp_local_env, "_cpp_moving_environment", None)
    if (
        cpp_local_owner is not None
        and hasattr(cpp_local_owner, "install_owner_local_optimize")
        and hasattr(cpp_local_owner, "run_owner_local_optimize_from_key")
    ):
        cpp_local_owner.install_owner_local_optimize(
            "reject-non-native",
            cpp_local_env,
            lambda _a, _b: pytest.fail("merge should not run after rejection"),
            lambda _guess: False,
            None,
            BlockTensor,
            None,
        ...
        assert cpp_local_owner.run_owner_local_optimize_from_key(
            "reject-non-native"
        ) is None
        cpp_local_env._sync_cpp_moving_environment_stats()
        assert cpp_local_env.moving_profile_stats[
            "owner_local_optimize_calls"
        ] == 1
        assert cpp_local_env.moving_profile_stats[
            "owner_local_optimize_rejections"
        ] == 1
        assert cpp_local_env.moving_profile_stats[
            "owner_local_optimize_rejected_reason"
        ] == "non_native_site_tensor"
        assert cpp_local_env.moving_profile_stats[
            "cpp_moving_environment_owner_local_optimize_record_hits"
        ] == 1
        assert cpp_local_env.moving_profile_stats[
