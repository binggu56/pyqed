...
def test_run_time_su2_selection_rebuilds_spin_orbital_hamiltonian_as_spatial_reduced():
    ...
    assert dmrg.dmrg.backend == "nonabelian"
...
def test_su2_cpp_owner_executes_cold_half_sweeps_without_python_bond_callbacks():
    ...
    assert owner["site_merge_calls"] == 10
    assert owner["active_bond_cpp_splits"] == 10
...
def test_su2_block2_cpp_owner_avoids_transformed_kernel_build():
    ...
    assert all(
        objective["cpp_davidson_kind"] == "cpp_su2_active_canonical_solve"
        for objective in objectives
    )
    assert all(objective["cpp_active_solution_owned"] for objective in objectives)
    assert all(
        objective["direct_complementary_action_executor"]
        for objective in objectives
    )
    assert all(objective["coupled_projection_in_cpp"] for objective in objectives)
    assert all(
    ...
    assert owner_stats["borrowed_factor_pool_bytes"] == 0
    ...
    assert max(float(objective["residual"]) for objective in objectives) < 1.0e-8
    assert max(
        abs(
...
def test_su2_block2_state_average_routes_through_operator_table():
    ...
    dmrg.run(
        ...
        local_basis_policy="block2_like",
        fully_reduced_state_average=False,
...
def test_spatial_su2_dmrg_supports_state_average_roots():
    ...
    assert str(objective["packed_matvec_backend"]).startswith("rank-coupled-")
