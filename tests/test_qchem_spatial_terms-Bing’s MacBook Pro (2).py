...
from pyqed.mps.abelian_direct import (
    ...
    abelian_safe_two_site_layout_map_from_layout,
...
def test_qchem_spatial_abelian_defaults_to_grouped_mpo():
    class Mol:
        spin = 0
    ...
    assert dmrg.spatial_abelian_mpo == "grouped"
...
def test_abelian_contextual_boundary_batch_tracks_packed_payloads():
    ...
    assert builder.left_packed_boundary_table.batch_resolves == 2
    assert builder.right_packed_boundary_table.batch_resolves == 2
    assert builder.left_packed_boundary_table.hits == 2
    assert builder.right_packed_boundary_table.hits == 1
    assert builder.left_packed_boundary_table.last_batch_hits == 2
    assert builder.right_packed_boundary_table.last_batch_hits == 1
...
def test_abelian_planned_direct_entries_materialize_only_when_needed():
    ...
    assert entries.local_left_ids == (0, 1)
    assert entries.local_right_ids == (0, 0)
...
def test_abelian_safe_two_site_layout_helpers_are_block_data_only():
    ...
    layout = tuple(
        ...
        for key, block in sorted(proto.data.items(), key=lambda item: item[0])
    )
    assert (
        abelian_safe_two_site_layout_map_from_layout(layout, w_sites=(w_left, w_right))
        == layout_map
    )
