...
def test_rtldr_metric_is_identity_and_same_grid_overlap_stays_identity():
    _prefer_source_package()
    from pyqed.namd.rtldr import RetainedStateRTLDR
    ...
    solver = RetainedStateRTLDR(grid, kinetic, electronic_hamiltonian)
...
def test_rtldr_identical_electronic_frames_reduce_to_plain_nuclear_kinetic():
    _prefer_source_package()
    from pyqed.namd.rtldr import RetainedStateRTLDR
    ...
    solver = RetainedStateRTLDR(grid, kinetic, electronic)
...
def test_rtldr_offgrid_overlap_tracks_relative_electronic_phase():
    _prefer_source_package()
    from pyqed.namd.rtldr import RetainedStateRTLDR
...
def test_rtldr_accepts_index_coordinate_time_callable():
    _prefer_source_package()
    from pyqed.namd.rtldr import RetainedStateRTLDR
...
def test_rtldr_builds_frames_from_overlap_and_uses_state_space_hamiltonian():
    _prefer_source_package()
    from pyqed.namd.rtldr import RetainedStateRTLDR, frames_from_overlap
    ...
    solver = RetainedStateRTLDR(grid, kinetic, electronic, initial_frames=frames)
    c0 = np.zeros((grid.size, 2), dtype=complex)
    c0[1, 0] = 1.0
