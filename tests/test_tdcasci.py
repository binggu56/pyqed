...
def test_tdcasci_validates_time_grid_inputs():
    td = TDCASCI(_h2_casci(nstates=2))

    for dt in (0.0, -0.1, np.inf, -np.inf, np.nan, "invalid"):
        with pytest.raises(ValueError, match="dt"):
            td.run(dt=dt, nsteps=1)

    for nsteps in (-1, 1.5, True, "2"):
        with pytest.raises(ValueError, match="nsteps"):
            td.run(dt=0.05, nsteps=nsteps)


def test_tdcasci_accepts_zero_steps_as_an_initial_state_trajectory():
    td = TDCASCI(_h2_casci(nstates=2))

    traj = td.run(dt=0.05, nsteps=0)

    np.testing.assert_array_equal(traj.times, np.array([0.0]))
