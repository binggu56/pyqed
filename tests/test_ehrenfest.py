...
def test_ehrenfest_run_records_final_partial_output_block():
    ed = Ehrenfest(ndim=1, ntraj=1, nstates=2, mass=1.0, nac_driver=_harmonic_driver)
    ed.sample(init_state=0, x0=[0.1], ax=8.0)
    ed.run(dt=0.01, nt=7, nout=3)

    expected_nsave = 1 + 3
    assert ed.times.shape == (expected_nsave,)
    np.testing.assert_allclose(ed.times, np.array([0.0, 0.03, 0.06, 0.07]))
    np.testing.assert_allclose(ed.norm_history, 1.0, atol=1e-10)
...
@pytest.mark.parametrize("electronic_representation", ["overlap", "local_diabatic_overlap"])
def test_pyscf_tddft_ehrenfest_overlap_smoke(electronic_representation):
    pytest.importorskip("pyscf")
    from pyscf import gto
    ...
    ed.run(dt=0.05, nt=6, nout=2, electronic_representation=electronic_representation)
...
def test_native_tddft_driver_point_data_uses_same_analytic_gradient_path():
    pytest.importorskip("pyscf")
    ...
    driver = TDDFTDriver(mol, 2, xc='lda')
    driver._finite_difference_gradients = lambda coords: (_ for _ in ()).throw(
        AssertionError("point_data should not fall back to finite differences here")
    )

    point = driver.point_data(mol.atom_coords())

    assert point['energies'].shape == (2,)
    assert point['grads'].shape == (2, 6)
    assert np.linalg.norm(point['grads'][1]) > 0.0
