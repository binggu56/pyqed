...
sets one global approximate workspace budget shared by all active q workers;
it is not a per-worker allowance.  ``stream_pair_batch_size`` can set an
explicit pair count for benchmarking or tightly controlled runs.
``folded_batch_mb`` independently bounds the live auxiliary slice of the BvK
folded tensor.  Lowering either budget reduces peak memory at the cost of more
pair-stream or auxiliary-shell batches.

``build(workers=N)`` treats :math:`N` as the requested q-block concurrency.
It selects the effective outer and inner concurrency so that

.. math::

   W_q \leq N,
   \qquad
   W_q W_{\rm inner} \leq N_{\rm CPU}.

The default target is three inner workers per active q block; set
``gdf_inner_worker_target`` or ``PYQED_GDF_INNER_WORKER_TARGET`` to tune that
target.  Canonical q sources are completed before opposite-q blocks are
derived, which avoids worker slots waiting recursively for their source.  The
global pair-stream budget is divided among active q workers,

.. math::

   M_{\rm active}=\frac{M_{\rm pair}}{W_q},
   \qquad
   \sum_{w=1}^{W_q}M_w\leq M_{\rm pair},

so increasing ``workers`` does not multiply the configured memory ceiling.  A
serial build can still group multiple complete q blocks into one shared
short-range traversal.

For a Gamma-commensurate uniform mesh, sufficiently large builds fold the
short-range image lattice into one Born--von Karman tensor before applying the
Bloch phases.  Write the image indices as
:math:`\mathbf L=\boldsymbol\ell+\mathbf n\mathbf N` and
:math:`\mathbf R=\mathbf r+\mathbf m\mathbf N`, where
:math:`\boldsymbol\ell,\mathbf r` label the :math:`N_k` primitive cells in the
BvK mesh.  The folded tensor is

.. math::

   T^{P\mu\nu}_{\boldsymbol\ell\mathbf r}
   = \sum_{\mathbf n,\mathbf m\in\mathbb Z^3}
     \left(P\mathbf 0\middle|\mu_{\boldsymbol\ell+\mathbf n\mathbf N}
     \nu_{\mathbf r+\mathbf m\mathbf N}\right)_{\rm SR},

and all ordered k-pair blocks follow from the six-dimensional discrete
transform

.. math::

   B^{P}_{\mu\nu}(\mathbf k,\mathbf k')
   = \sum_{\boldsymbol\ell,\mathbf r}
     e^{-i\mathbf k\cdot\mathbf L_{\boldsymbol\ell}}
     T^{P\mu\nu}_{\boldsymbol\ell\mathbf r}
     e^{+i\mathbf k'\cdot\mathbf L_{\mathbf r}}.

The implementation partitions the auxiliary shells into contiguous batches
:math:`\mathcal A_b`, evaluates one folded slice
:math:`T^{P\mu\nu}_{\boldsymbol\ell\mathbf r}` with
:math:`P\in\mathcal A_b` at a time, and applies FFTs over the left and right
BvK axes.  Only requested :math:`(\mathbf k,\mathbf k')` rows are selected from
the live FFT block.  They are transformed into the fitting basis and added to
bounded q-resolved :math:`j_{3c}` files,

.. math::

   B^A_{\mu\nu}(\mathbf k,\mathbf k')
   \mathrel{+}=
   \sum_{P\in\mathcal A_b}U_{PA}
   B^P_{\mu\nu}(\mathbf k,\mathbf k').

After the short-range stream, reciprocal and :math:`G=0` compensation terms
are added to one q block at a time.  The complete block is whitened once,

.. math::

   L^a_{\mu\nu}(\mathbf k,\mathbf k+\mathbf q)
   =\sum_A\left[J^{-1/2}(\mathbf q)\right]^*_{Aa}
   B^A_{\mu\nu}(\mathbf k,\mathbf k+\mathbf q),

written to persistent cderi storage, and its raw q file is deleted.  Thus the
live folded workspace obeys the configured bound

.. math::

   \max_b\left[
   |\mathcal A_b|N_k^2N_{\rm AO}^2\,{\rm sizeof}(\mathbb R)
   \right]\lesssim M_{\rm folded}.

Only canonical factors occupy persistent storage.  If Hermitian or
opposite-momentum symmetry gives

.. math::

   L^a_{\mu\nu}(\mathbf k',\mathbf k)
   =\left[L^a_{\nu\mu}(\mathbf k,\mathbf k')\right]^*,

the derived cache entry is an alias to the canonical factor rather than a
second array or disk file.  The public ``build`` call is a transaction guarded
by one backend-level lock.  A Python interruption or factor-write failure
removes partial files, q spools, newly created factors, and transient GDF
caches while preserving factors that existed before the call.  Concurrent
callers therefore share one completed transaction instead of racing to write
the same cderi paths.

The Cartesian-to-spherical auxiliary transformation uses shell-local blocks
rather than a dense contraction,

.. math::

   B^A_{\mu\nu}=\sum_P U_{PA}B^P_{\mu\nu}.

No global
:math:`N_k^2N_{\rm aux}N_{\rm AO}^2` complex transformed-pair tensor is
allocated.  All temporary q files are deleted at the end of ``build`` and on
exceptions.  Nonuniform, shifted, or noncommensurate meshes use the direct
shared-phase path.  This is an architecture-level adaptation of the periodic
GDF formulation cited below, not a claim that every PySCF ``libpbc`` kernel is
reproduced.

Same-k ``q=0`` blocks use Hermitian packed storage, opposite q blocks are
generated by conjugation, and GW/BSE reuse the persisted SCF factors.  Call
...
PyQED GDF therefore evaluates one canonical source for each unordered
...
* Analytic-continuation GW contracts the imaginary-axis response in the
  auxiliary basis.  With a flattened transition label
  :math:`t=(\mathbf k,i,a)` and
  :math:`\Delta_t=\epsilon_{i\mathbf k}-
  \epsilon_{a,\mathbf k+\mathbf q}`, the implemented polarizability is

  .. math::

     \Pi_{PQ}(\mathbf q,i\omega)
     = \frac{4s_d}{N_k}\sum_t
       \frac{\Delta_t}{\omega^2+\Delta_t^2}
       L_{Pt}(\mathbf q)L^*_{Qt}(\mathbf q),

  followed by

  .. math::

     W^c(\mathbf q,i\omega)
     = [I-\Pi(\mathbf q,i\omega)]^{-1}-I.

  The target-orbital contraction is evaluated for all requested orbitals in
  one batch,

  .. math::

     W_{mn}(\mathbf k,\mathbf q,i\omega)
     = \frac{s_d m_{\mathbf q}}{N_k}
       \sum_{PQ}
       [L^P_{mn}(\mathbf k,\mathbf q)]^*
       W^c_{PQ}(\mathbf q,i\omega)
       L^Q_{mn}(\mathbf k,\mathbf q),

  where :math:`s_d` is ``direct_scale`` and :math:`m_{\mathbf q}` is the
  optional time-reversal multiplicity.  Each generated q-resolved MO factor
  is released after its contribution; factors explicitly present before the
  call remain cached.  ``result.info["ac_profile"]`` records factor,
  polarizability, inversion, self-energy, finite-size, and peak-factor data.
* The built-in finite-size head caches the k-independent Gaussian derivative
  terms and applies only the Bloch phases at each k point,

  .. math::

     D^\alpha_{\mu\nu}(\mathbf k)
     = -\sum_{\mathbf L}e^{i\mathbf k\cdot\mathbf L}
       \left\langle\chi_{\mu\mathbf 0}\middle|
       \partial_{B_\alpha}\chi_{\nu\mathbf L}\right\rangle.

  This is an algebraically identical reorganization of the analytic Gaussian
  derivative, not a finite-difference approximation.

The imaginary-axis/two-pole route is an adaptation of H. N. Rojas,
R. W. Godby, and R. J. Needs, *Phys. Rev. Lett.* **74**, 1827 (1995),
doi:10.1103/PhysRevLett.74.1827, using the periodic PySCF continuation
convention.  It is intended for quasiparticle roots of closed-shell
insulators.  It is not a full-frequency satellite solver and does not support
metals.
...
The current shifted-mesh release qualification uses the same solid-adapted
basis on :math:`2\times2\times2` points.  The worst auxiliary-projected pair
metric residual is :math:`1.70\times10^{-7}`, and the propagated errors are

.. math::

   \max|\Delta J|=1.34\times10^{-3}\ {\rm meV},\qquad
   \max|\Delta K|=3.27\times10^{-4}\ {\rm meV},

.. math::

   |\Delta E_{\rm KRHF}|=1.02\times10^{-8}\ E_h,\qquad
   \max|\Delta E^{\rm QP}_{\rm same\ factor}|=
   5.84\times10^{-5}\ {\rm meV}.

The independent end-to-end Gamma-centered :math:`2\times2\times2` KGW
comparison builds and runs each package in a fresh process.  It gives a KRHF
total-energy residual of :math:`8.89\times10^{-9}\ E_h`, a maximum
mean-field orbital residual of :math:`6.18\times10^{-4}` meV, and

.. math::

   \max|\Delta E^{\rm QP}|=5.79\times10^{-3}\ {\rm meV},\qquad
   \operatorname{RMS}(\Delta E^{\rm QP})=5.06\times10^{-3}\ {\rm meV}.

PyQED spends 4.53, 1.10, and 3.08 s in GDF, KRHF, and GW, respectively;
PySCF spends 2.60, 0.76, and 9.15 s.  The corresponding isolated process
peak increments are 376 and 531 MB.  These figures compare the packages'
actual GW conventions and finite-size implementations, rather than reusing a
common factor tensor.

A diffuse shifted :math:`2\times1\times1` H2/cc-pVDZ case gives J, K, KRHF,
and same-factor GW residuals of 0.000221, 0.000407, 0.0000187, and 0.000133
meV.  A primitive bcc-Li :math:`2\times2\times2` calculation exercises the
zero-temperature metallic ensemble: six frontier orbitals are fractional and
their splitting is :math:`5.25\times10^{-12}\ E_h`.  The PyQED/PySCF KRHF
energy residual is :math:`9.77\times10^{-10}\ E_h`.  The transition-factor
diagnostic uses a separately identified integer determinant because the GW/BSE
transition basis does not yet accept fractional occupations.
...
:math:`3^3\rightarrow4^3` shift is still :math:`0.72256` eV.  The PyQED total
times are :math:`82.8`, :math:`408.1`, and :math:`2330.2` s, compared with
:math:`65.9`, :math:`101.6`, and :math:`301.1` s in PySCF.  At :math:`4^3`,
PyQED GDF construction takes :math:`1965.2` s and is the dominant performance
target.  A predictive gap requires a larger symmetry-reduced mesh and a
converged orbital basis.

With bounded two-level scheduling, shared-q short-range traversal, shell-block
auxiliary transforms, and BvK folding, strict unpruned GDF-only reruns reduce
the :math:`2^3`, :math:`3^3`, and :math:`4^3` build times from
:math:`66.55`, :math:`335.41`, and :math:`1965.22` s to :math:`25.90`,
:math:`61.35`, and :math:`128.04` s.  The measured speedup,

.. math::

   S(n)=\frac{t_{\rm original}(n)}{t_{\rm current}(n)},

is :math:`2.57`, :math:`5.47`, and :math:`15.35`, respectively.  The SCF
energies are unchanged at the precision stored in the benchmark JSON files.
The matched PySCF GDF times are :math:`47.49`, :math:`52.01`, and
:math:`160.53` s: PyQED is faster at :math:`2^3` and :math:`4^3`, while PySCF
is :math:`9.33` s faster at :math:`3^3`.  At :math:`4^3`, the one-time folded
image build and FFT take :math:`28.02` and :math:`6.63` s; the next two
short-range cache selections take only :math:`0.239` and :math:`0.161` s.

The isolated end-to-end :math:`4^3` comparison uses ``--isolated-memory`` so
PyQED and PySCF execute sequentially in fresh processes.  This prevents one
package's live reference, allocator arenas, or already-committed pages from
hiding the other's allocations.  In the pre-sharding baseline, the isolated
PyQED phase times were :math:`212.86`, :math:`33.71`, and :math:`14.94` s for
GDF, KRHF, and :math:`G_0W_0`; the PySCF times were :math:`147.32`,
:math:`21.25`, and :math:`97.03` s.

The current strict scheduler profile adds a grouped short-range auxiliary
metric, symmetry aliases, q-sharded disk cderi, and q-major KRHF contractions.
At :math:`10^{-12}` precision with a global 128 MB pair-stream budget, fully
disk-backed PyQED GDF takes :math:`122.09` s and the following KRHF takes
:math:`34.48` s, for :math:`156.57` s combined.  The matched PySCF GDF plus
KRHF reference takes :math:`168.57` s.  The current PyQED reference build is
therefore :math:`1.08` times faster end to end, while GDF construction alone
is :math:`1.21` times faster than PySCF and :math:`1.74` times faster than the
pre-sharding PyQED baseline.

The PyQED VBM and CBM residuals remain :math:`-0.00295` and
:math:`+0.00442` meV, while the Gamma QP gaps are :math:`13.0327370` and
:math:`13.0327297` eV.  The current fresh PyQED scheduler process starts at
:math:`0.245` GB, peaks at :math:`2.200` GB, and retains :math:`1.941` GB
above baseline after KRHF.  Relative to the pre-sharding isolated process,
peak RSS falls by :math:`0.518` GB and the retained increment falls by
:math:`0.534` GB.  PySCF peaks at :math:`1.893` GB, with peak and retained
increments of :math:`1.656` and :math:`1.254` GB.  Current PyQED therefore
uses :math:`1.16` times PySCF's absolute peak and :math:`1.18` times its peak
increment.  The remaining retained-RSS gap is allocator residency rather than
live Python GDF caches: non-factor caches occupy :math:`40.26` MB.

The :math:`4096` logical cderi blocks contain :math:`2016` conjugate symmetry
aliases.  Their canonical disk payload is :math:`453.20` MB in only :math:`99`
files.  The raw q spool is bounded to :math:`15.36` MB per q block and
:math:`499.20` MB total temporary storage; the live folded batch is
:math:`127.80` MB and the global transformed-pair storage diagnostic remains
zero.

The earlier shared-process sampler is unsuitable for package attribution: it
kept both references alive and allowed later allocations to reuse committed
memory, making the PySCF GW increment appear artificially small.  Use
``--isolated-memory`` for package-to-package RSS comparisons.  A standalone
GDF scheduler profile further separates the memory/time tradeoff.  Those
instrumented runs retain additional diagnostics and are not directly
comparable to the isolated package peaks above.  Before the q-file pipeline,
the default 128 MB settings took :math:`135.10` s at :math:`4.32` GB peak RSS,
while a 64 MB pair-stream budget took :math:`165.34` s at :math:`3.72` GB.
With the new pipeline, the :math:`10^{-8}` 128 MB run takes :math:`115.27` s
at :math:`2.60` GB peak RSS.  Its persistent non-factor caches occupy only
:math:`33.45` MB, and the global transformed-pair storage diagnostic is zero.

Generate one scheduler profile and its per-q diagnostic figure with

.. code-block:: console

   PYTHONPATH=. OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 \
     VECLIB_MAXIMUM_THREADS=1 NUMEXPR_NUM_THREADS=1 \
     python examples/pbc_gdf_build_benchmark.py \
       --case diamond --kmesh 4,4,4 --gamma-centered \
       --precision 1e-12 --aux-min-exponent 0.075 \
       --metric-tol 1e-13 --workers 8 \
       --storage disk --max-memory-mb 0 \
       --output /private/tmp/pbc_diamond_gdf_444.json \
       --figure /private/tmp/pbc_diamond_gdf_444

Compare a series of previous and current JSON files with
``examples/plot_pbc_gdf_speedup.py``.  Pass the full comparison files again
through ``--reference`` to overlay their PySCF GDF timings.

Reproduce the isolated end-to-end memory comparison with
...
.. code-block:: console
   ...
   PYTHONPATH=. OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 \
     VECLIB_MAXIMUM_THREADS=1 NUMEXPR_NUM_THREADS=1 \
     python examples/pbc_gw_pyscf_benchmark.py \
       --case diamond --kmesh 4,4,4 --gamma-centered --gamma-only \
       --precision 1e-12 --reference-precision 1e-12 \
       --aux-min-exponent 0 --metric-tol 1e-13 \
       --pyscf-metric-eig --ac-nw 24 --workers 8 \
       --isolated-memory --skip-aligned \
       --output /private/tmp/pbc_diamond_gamma_444_gw_isolated.json \
       --figure /private/tmp/pbc_diamond_gamma_444_gw_isolated
...
The native GDF--KRHF and diagonal kGW path is production-qualified within its
restricted three-dimensional scope.  The overall periodic GW/BSE package is
not yet a general production implementation.  Important limitations are
explicit:

* spin-restricted references only; odd-electron primitive cells require an
  even total electron count over the sampled k mesh and
  ``occupation_mode="fractional"``.  Zero-temperature fractional frontier
  occupations are available for metallic degeneracies, but finite-temperature
  smearing and unrestricted metals are not implemented;
* native Ewald periodic references only;
* no native two-dimensional truncated-Coulomb GDF; a vacuum-padded 3D cell is
  a 3D periodic approximation, not the missing 2D solver;
...
* ``coulomb_component="gdf"`` is a native auxiliary-basis GDF backend with
  ...
  representation-matched PySCF GDF factors for H2, LiH, bcc Li, He, diamond,
  and BN.  Group-closed shifted and nonrectangular meshes use the direct Bloch
  fallback.  Under the documented strict 64-k diamond settings, disk-backed
  PyQED GDF plus KRHF takes 156.57 s versus 168.57 s for PySCF.  Small-system
  build times still vary by basis and mesh;
