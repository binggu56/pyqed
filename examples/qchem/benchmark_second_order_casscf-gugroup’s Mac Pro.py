...
def _parse_modes(text):
    ...
    allowed = {"qn", "uncoupled", "partial", "full", "relaxed_fd"}
...
def _internal_preopt_stats(history):
    ...
    if guarded:
        ...
        guard_delta = np.nan
    return attempted, accepted, guard_rejected, guard_delta
...
def _run_mode(args, mf, mo_guess, case, mode):
    ...
    try:
        ...
        ip_attempt, ip_accept, ip_guard_reject, ip_guard_delta = _internal_preopt_stats(
            mc.internal_preopt_history
        )
        result.update(
            {
                "ok": True,
...
def _print_table(rows, e_initial):
    ...
    header = (
        ...
        "ah_bad  ip_a ip_ok ip_grd  ip_dguard   grad        step"
    )
    if show_pyscf_delta:
...
def main():
    ...
    parser.add_argument(
        ...
        choices=("diagonal", "finite_difference"),
