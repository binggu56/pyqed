#!/usr/bin/env python3
"""Run a tiny electrostatically embedded QM/MM water-box MD example.

The solute is an H2 molecule treated with PyQED RHF.  The solvent is a small
rigid TIP3P water box treated by the MD ``MM`` calculator.  This example is a
smoke benchmark for the end-to-end workflow, not a production sampling setup.
...
def parse_args():
    ...
    parser.add_argument("--timestep-fs", type=float, default=0.05, help="Timestep in fs.")
    parser.add_argument("--box-angstrom", type=float, default=8.0, help="Cubic box length in Angstrom.")
    ...
    parser.add_argument("--seed", type=int, default=7, help="Random seed for initial velocities.")
    parser.add_argument("--output-dir", default="md_outputs", help="Output directory.")
    return parser.parse_args()


def h2_solute(center, bond_length=1.4):
    center = np.asarray(center, dtype=float)
    dz = np.array([0.0, 0.0, 0.5 * bond_length])
    positions = np.vstack([center - dz, center + dz])
    solute = Atoms([["H", tuple(positions[0])], ["H", tuple(positions[1])]])
...
def main():
    ...
    solute, solute_positions = h2_solute(center=(0.5 * box_length, 0.5 * box_length, 0.5 * box_length))
    system = solvate_box(
        solute=solute,
        box_size=(box_length, box_length, box_length),
        spacing=spacing,
        min_distance=2.2 / au2angstrom,
    ...
    qm_indices = np.array([0, 1], dtype=int)
    mm_indices = np.arange(2, len(system), dtype=int)
    system.calc = QMMM(
        qm=qm_h2_reference(solute_positions),
    ...
    trajectory_path = output_dir / "qmmm_water_box.xyz"
    energy_path = output_dir / "qmmm_water_box_energy.dat"
    dynamics = Langevin(
        system,
        timestep=timestep,
