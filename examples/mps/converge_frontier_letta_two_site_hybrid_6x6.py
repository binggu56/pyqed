...
def converge(
    *,
    ...
    ng_metric_tol = float(ng_metric_tol)
    if not np.isfinite(ng_metric_tol) or ng_metric_tol < 0.0:
        raise ValueError("ng_metric_tol must be finite and nonnegative")
    ...
    if resumed:
        ...
        if (
            abs(float(state.expectation()) - metadata["energy"])
...
def main():
    ...
    parser.add_argument("--ng-metric-tol", type=float, default=1.0e-12)
    ...
    converge(
        ...
        ng_metric_tol=args.ng_metric_tol,
