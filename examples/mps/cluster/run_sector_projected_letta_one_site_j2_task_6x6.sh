...
if not math.isclose(actual, expected, rel_tol=0.0, abs_tol=1.0e-12):
    raise SystemExit(f"source J2={actual} does not match manifest J2={expected}")' \
    "$source_result" "$j2"

starting_directional_sweep=${LETTA_STARTING_DIRECTIONAL_SWEEP:-auto}
...
value = result.get("next_directional_sweep")
if value is None and protocol.get("directional_sweep") is not None:
    value = int(protocol["directional_sweep"]) + int(
        result.get("directional_sweeps_completed", 1)
    )
if value is None and protocol.get("sweep_offset") is not None:
    value = int(protocol["sweep_offset"]) + int(
        result.get("directional_passes_completed", 0)
    )
if value is None:
    raise SystemExit("source checkpoint does not define the next directional sweep")
print(int(value))' \
            "$source_result"
    )
fi
if ! [[ $starting_directional_sweep =~ ^[0-9]+$ ]]; then
    echo "invalid starting directional sweep: $starting_directional_sweep" >&2
    exit 2
fi
...
echo "exact U(1) one-site LETTA 6x6 task=$task_index J2/J1=$j2 host=$(hostname) start=$(date -Iseconds)"
...
exec "$python" -u \
    "$PYQED_ROOT/examples/mps/converge_sector_projected_letta_6x6.py" \
    --source-result "$source_result" \
    --source-snapshot "$source_snapshot" \
    --output "$run_dir/result.json" \
    --snapshot "$run_dir/state.npz" \
    --maximum-passes "$maximum_directional_passes" \
    --sweep-offset "$starting_directional_sweep" \
