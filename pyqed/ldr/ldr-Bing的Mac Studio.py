...
class LDR2_Jacobi(LDR2):
    """
    2D LDR solver for curvilinear coordinates with metric g_{\mu \nu}
    The kinetic energy operator reads 
    
    The coordinates are `r, \theta`. 
    ...
    def __init__(self, domains, levels, dvr_types, mass=None, ndim=2):
        ...
        if mass is None:
            raise ValueError('The mass of y should be a callback.')
            
        self.mass = mass
        self.moment_of_inertia = mass[1]
    ...
    def buildK(self):
        ...
        # x is a normal coordinate
        Tx = kinetic(x, mass=mx, dvr=self.dvr_type[0])
        
        expKx = scipy.linalg.expm(-1j * Tx * dt)
        
        # y is an angular coordinate
        Ty = kinetic(self.x[d], mass=1, dvr=self.dvr_type[1])
        
        Ty = contract('n, ij -> nij', 1/(2 * I), Ty, )
