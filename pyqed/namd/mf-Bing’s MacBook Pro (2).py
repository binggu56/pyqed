...
class TDDFTDriver:
    ...
    def point_data(self, coords):
        ...
        _, ks, td = self._build_state_point(coords)
        energies = self._state_energies(ks, td)
        grads = self._pyscf_gradients(ks, td) if self.backend == 'pyscf' else self._finite_difference_gradients(coords)
        return {
            'coords': coords,
            'ks': ks,
            'td': td,
...
class Ehrenfest:
    ...
    def run(self, dt=0.01, nt=10, nout=1, method='euler', force_driver=None,
            electronic_representation='nac', *args):
        """
        ...
        if electronic_representation == 'adiabatic_nac':
            electronic_representation = 'nac'
        if electronic_representation != 'nac':
            raise ValueError(
                "Ehrenfest.run only supports electronic_representation='nac'."
            )
...
class TDDFTEhrenfest(Ehrenfest):
    ...
    def run(self, dt=0.01, nt=10, nout=1, method='euler', force_driver=None,
        ...
        if electronic_representation == 'adiabatic_nac':
            electronic_representation = 'nac'
        elif electronic_representation == 'local_diabatic_overlap':
            electronic_representation = 'overlap'

        if electronic_representation == 'nac':
            return super().run(dt=dt, nt=nt, nout=nout, method=method, force_driver=force_driver, *args)
        if electronic_representation != 'overlap':
            raise ValueError(
                "electronic_representation must be 'nac' or 'overlap'."
            )
        ...
        for step in range(nt // nout):
            for _ in range(nout):
                for traj in self.trajs:
                    ...
                    old_point = traj.state_point
                    new_point = driver.point_data(traj.x.reshape(self.natom, 3))
                    overlap = driver.state_overlap(
                        old_point['coords'],
                        new_point['coords'],
                        ref_data=old_point,
                        other_data=new_point,
                    )
                    transform = self._unitarize_overlap(overlap)

                    h_old = np.diag(np.asarray(old_point['energies'], dtype=float))
                    h_new_ld = self._ld_hamiltonian(transform, new_point['energies'])
                    h_mid = 0.5 * (h_old + h_new_ld)

                    c_old_new = expm(-1j * h_mid * dt) @ traj.c
                    grad_ld = self._ld_gradient_matrices(transform, new_point['grads'])
                    traj.force = self._ld_force(c_old_new, grad_ld)

                    traj.c = transform.conj().T @ c_old_new
                    c_norm = np.linalg.norm(traj.c)
                    if c_norm > 0:
                        traj.c = traj.c / c_norm

                    traj.energy = new_point['energies']
                    traj.grad = new_point['grads']
                    traj.state_point = new_point
                    traj.overlap = overlap
                    traj.ld_transform = transform
                    traj.v = traj.p / mass
                    traj.p += dt2 * traj.force
