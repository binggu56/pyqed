...
def gdvr_det_overlap(left, right):
...
class GDVRFrame:
    ...
    def __init__(self, mf, *, field=None, interaction=None, cap=None, s_thresh=1.0e-12):
    ...
    def phase_energy(self, time=0.0, density=None):
        ...
        return float(obs["energy"] - np.dot(obs["field"], obs["dipole"]))
    ...
    def step(self, time, dt):
        ...
        self.det_phase *= np.exp(-1j * phase_energy * float(dt)) / raw_phase
...
class Solver:
    ...
    def _lpa_overlap_matrix(self):
        return LDRN.lpa_matrix(
            self.grid_shape,
            self._lpa_overlap_links(),
        )

    def overlap_matrix(self):
        if self.overlap_method == "full":
            return self._full_overlap_matrix()
        return self._lpa_overlap_matrix()

    def kinetic_matrix(self):
        return self.kinetic * self.overlap_matrix()

    def _lpa_sparse_kinetic_matrix(self):
        if self.overlap_method != "lpa":
    ...
    def propagation_hamiltonian(self, time=0.0, *, hamiltonian_method="dense"):
        ...
        if hamiltonian_method == "lpa-sparse":
            return self._lpa_sparse_kinetic_matrix()
    ...
    def static_ldr_hamiltonian(self, time=0.0, *, hamiltonian_method="dense"):
        ...
        h0 = 0.5 * (h0 + h0.conj().T)
        h0 += np.diag(self.electronic_energy_vector(time=time))
        return 0.5 * (h0 + h0.conj().T)

    def static_ldr_ground_state(self, time=0.0, *, hamiltonian_method="dense"):
        ...
        *,
        ...
        for step, time in enumerate(times):
            if progress_every and (step == 0 or step == nsteps or step % int(progress_every) == 0):
                print(f"[tdldr] propagated {step}/{nsteps}", flush=True)
