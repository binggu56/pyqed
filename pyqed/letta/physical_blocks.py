...
class PhysicalBlockLinearOperator:
    ...
    def matvec(self, vector) -> np.ndarray:
        return self._packed.matvec(vector)

    def matvecs(self, vectors) -> np.ndarray:
        """Apply all stored routes to several vectors in one grouped call."""
        return self._packed.matvecs(vectors)

    def diagonal(self) -> np.ndarray:
        """Return the exact flattened diagonal without materializing the matrix."""
        return self._packed.diagonal()
...
class PhysicalBlockGeneralizedProblem:
    ...
    def solve(self, initial_vector, **davidson_options):
        ...
        recycle_spaces = davidson_options.pop("recycle_spaces", None)
        recycle_prefix = tuple(
            davidson_options.pop("recycle_prefix", ())
        )
        executor = davidson_options.pop("executor", None)
        recycle_min_size = int(
            davidson_options.pop("recycle_min_size", 256)
        )
        preconditioner_option = davidson_options.pop("preconditioner", "auto")
        if isinstance(preconditioner_option, str):
            preconditioner_option = preconditioner_option.lower().replace("-", "_")
            if preconditioner_option not in {"auto", "jacobi", "none"}:
                raise ValueError(
                    "preconditioner must be 'auto', 'jacobi', None, or a callable."
                )
        elif preconditioner_option is not None and not callable(
        ...
        def solve_component(index_and_component):
            component_index, component = index_and_component
            ...
            if component_size <= dense_component_max_size:
                if reduced_hamiltonian_blocks is None:
                    dense_hamiltonian = np.column_stack(
                        [
                            hamiltonian_action(
                                np.eye(
                                    1,
                            ...
                            for column in range(component_size)
                        ]
                    )
                else:
                    ...
                    for row_slice, column_slice, block in (
                        reduced_hamiltonian_blocks
                    ):
