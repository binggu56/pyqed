...
class MPOFrontier:
    ...
    def hole_blocks(
        ...
        """Return a batch of fixed-physical virtual blocks."""
        site = int(site)
        bra_configurations = tuple(
            self._validated_physical_configuration(
                site,
                configuration,
                name="bra_configuration",
            )
            for configuration in bra_configurations
        )
        ket_configurations = tuple(
            self._validated_physical_configuration(
                site,
                configuration,
                name="ket_configuration",
            )
            for configuration in ket_configurations
        )
        ...
        if not bra_configurations:
            ...
            return np.empty((0, virtual_size, virtual_size), dtype=self.mpo_tensors[site].dtype)

        spectators = self.physical_groups[site][1:]
        optimized_pair = bool(
            len(spectators) == 1
            and not self.frontier_sites[site]
            and tuple(self.frontier_sites[site + 1]) == tuple(spectators)
            and spectators[0] in self.paired_sites
            and left.ndim == 3
            and right.ndim == 5
        )
        ...
        bra = np.asarray(bra_configurations, dtype=int)
        ket = np.asarray(ket_configurations, dtype=int)
        operators = np.moveaxis(
            self.mpo_tensors[site][:, :, bra[:, 0], ket[:, 0]],
            -1,
            0,
        )
        right_blocks = np.moveaxis(
            right[:, :, :, bra[:, 1], ket[:, 1]],
            -1,
            0,
        )
        ...
        virtual_size = self.virtual_bonds[site] * self.virtual_bonds[site + 1]
        return blocks.reshape(len(bra_configurations), virtual_size, virtual_size)
