...
class KGW:
    ...
    def _run_periodic(self, mo_energy=None, mo_coeff=None, method="g0w0", **kwargs):
        ...
        self._periodic_space = space
        from .self_energy import DiagonalSelfEnergyCache

        periodic_cache = kwargs.get("cache")
        if periodic_cache is None:
            periodic_cache = DiagonalSelfEnergyCache()
            kwargs["cache"] = periodic_cache
    ...
    def spectral_function(self, **kwargs):
        """Compute a molecular or k-resolved periodic GW spectral function."""

        if not self.periodic_backend:
            if self._gw is None:
                raise ValueError("Run KGW before computing a spectral function.")
            self.spectral_result = self._gw.spectral_function(**kwargs)
            return self.spectral_result
        if self.e_qp is None:
            raise ValueError("Run KGW before computing a spectral function.")

        from .pes import periodic_spectral_function
        from .self_energy import DiagonalSelfEnergyCache

        rebuild_space = any(
            key in kwargs for key in ("qpts", "occupation_tol", "occ_bands", "vir_bands")
        )
        qpts = kwargs.pop("qpts", "mesh")
        occupation_tol = kwargs.pop("occupation_tol", 1.0e-8)
        occ_bands = kwargs.pop("occ_bands", None)
        vir_bands = kwargs.pop("vir_bands", None)
        if self._periodic_space is not None and not rebuild_space:
            space = self._periodic_space
        else:
            space = self.transition_space(
                qpts=qpts,
                occupation_tol=occupation_tol,
        ...
        for key in inherited:
            if key not in kwargs and self.info.get(key) is not None:
                kwargs[key] = self.info[key]
        kwargs.setdefault("e_qp", self.e_qp)
        kwargs.setdefault("eta", self.eta)
        if self._periodic_cache is None:
            self._periodic_cache = DiagonalSelfEnergyCache()
        kwargs.setdefault("cache", self._periodic_cache)
        self.spectral_result = periodic_spectral_function(space, **kwargs)
        return self.spectral_result

    def experimental_pes(
        self,
        spectral_result=None,
        spectral_kwargs=None,
        **kwargs,
    ):
        """Compute a matrix-element-weighted periodic photoemission spectrum."""

        if not self.periodic_backend:
            raise NotImplementedError(
                "KGW.experimental_pes currently supports the periodic backend; "
                "use molecular GW full_spectrum() for molecular references."
            )
        if self.e_qp is None:
            raise ValueError("Run KGW before computing a photoemission spectrum.")
        if spectral_result is None:
            options = {"energy_reference": "fermi"}
            if spectral_kwargs is not None:
                options.update(dict(spectral_kwargs))
            spectral_result = self.spectral_function(**options)

        from .pes import periodic_photoemission_spectrum

        self.photoemission_result = periodic_photoemission_spectrum(
            self.kref,
            spectral_result,
            **kwargs,
        )
        return self.photoemission_result

    def photoemission_spectrum(self, **kwargs):
        """Alias for :meth:`experimental_pes`."""

        return self.experimental_pes(**kwargs)
