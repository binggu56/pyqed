...
class NEGF:
    def __init__(self, nt, ntau=0, size=1, sign=-1, dt=None):
        """
        ...
        nt : TYPE
            DESCRIPTION.
        ntau : TYPE, optional
            DESCRIPTION. The default is 1. TBE.
        size : TYPE, optional
            DESCRIPTION. The default is 1.
        sign : TYPE, optional
            DESCRIPTION. The default is -1.
...
def distribution_exp_eq(beta, tau, omega, sign):
 ...
 # template <typename T,class dos_function>
def green_single_mode(omega0, dt, nt, sign=-1, mu=0):
    """
    ...
    G = NEGF(dt=dt, nt=nt, sign=sign,  size=1)
    # dtau = G.dtau
...
def green_from_H_const(H0, nt, dt, sign=-1, mu=0):
    # std::complex<T> iu = std::complex<T>(0.0, 1.0);
    # int nt=G.nt(),ntau=G.ntau();
    # int size=G.size1();
    # int sign=G.sig();
   ...
   #      Ut.append(U.copy())
    ...
    # if sign==-1:
    #     value = evec0 @ diag(eval0m < mu) @ dag(evec0)
    # elif(sign==1):
    #     value = -1.0*evec0 @ diag(bose(beta,eval0m)) @ dag(evec0)

    value = evec0 @ diag((eval0m < mu).astype(int)) @ dag(evec0)


    G = NEGF(dt=dt, nt=nt, size=size)
...
def green_from_H(H0, nt, dt, sign=-1, mu=0):
    # std::complex<T> iu = std::complex<T>(0.0, 1.0);
    # int nt=G.nt(),ntau=G.ntau();
    # int size=G.size1();
    # int sign=G.sig();
    ...
    if isinstance(H0, np.ndarray):
        return green_from_H_const(H0, nt, dt)
...
def bubble(A, B, vertex):
    ...
    .. math::
        ...
        C_{c1, c2}(t1,t2) = -1j  V^{c1}_{b1, a1) A_{a1,a2}(t1,t2)  B_{b2,b1}(t2,t1) V^{c2}_{b2, a2}

    Should I add -i or - 1 or nothing???
    ...
    #         c.set_tv(i, j, -1 * trace(v @ atv @ conj(v) @ bvt))
...
def trapezoid_half_edge(f):
  ...
  # #----------------------------------------------------------------------------
    ...
    for k in range(1, len(f)): integral+=f[k]
...
class Keldysh_derivative:
    def __init__(self, nt, size=1, sign=-1, dt=None):
        """
        The time domain used to represent the GF are
        ...
        self.retarded = np.zeros((nt+1, size, size), dtype=complex)
        self.lesser = np.zeros((nt+1, size, size), dtype=complex)
...
class KeldyshSolver:
    def __init__(self, h, dt, nt, omegac):
        """
    ...
    def initiate(self):
        ...
        G0 = green_from_H(h, dt=self.dt, nt=self.nt, sign=-1)

        D0 = green_single_mode(w, dt=dt, nt=nt, sign=1)
    ...
    def run(self, vertex):
        # we only include the Hartree term, so the fermison Dyson equaiton is time-local
        # but the photon Dyson equation is time-nonlocal
        
        G0, D0 = self.initiate()
        chi = bubble(G, G, vertex)
        # photon Hamiltonian H = h_{ij}a_i^\dag a_j 
        h = np.zeros((1,1))
        h[0,0] = self.omegac 
        
        D = volterra_intdiff(nt=nt, dt=dt, h=h, K=chi)
    
        return G, D
    


def volterra_intdiff_timestep(dt, n, h, K, G, G_der):
    """
    /*----------------------------------------------------------------------------
   ...
   #                    -i\int_{t'}^{t} ds K^{R}(t,s) G^{R}(s,t')
    ...
    # G = NEGF(nt, dt) # the following code only works for size == 1 now
    
    size = G.size
    idm = eye(size)
    
    G.retarded[n, n] = -1j * idm # represented in an orthonormal basis set, e.g, eigenstates of H0
    
    G_der_new = Keldysh_derivative(nt, size=G.size)
    ...
    for j in range(n):
        ...
        KxG = [K.retarded[n][l] @ G.retarded[l][j] for l in range(j, n+1)]

        G_der_new.retarded[j] = -1j*dt * trapezoid_half_edge(KxG)

        G.retarded[n][j] += 0.5*dt*G_der_new.retarded[j]

        G.retarded[n][j] = np.linalg.solve( idm +0.5j*dt*h[n] + 0.25j*dt*dt * K.retarded[n][n], G.retarded[n, j])
  ...
  # }
   ...
   # G^{<}(t_{n},t_{j}), d/dt G^{<}(t_{n},t_{j})
    ...
    for j in range(n):
        G.lesser[n][j] = G.lesser[n-1][j] + 0.5 * dt * G_der.lesser[j]

        # for l in range(ntau+1):
        #     KxG[l] = K.left_mixing[n][l] @ conj(left_mixing[j][ntau-l])
        # G_der_new.lesser[j]= -1j*(-xj) * dtau * trapezoid(KxG,0, ntau)

        KxG = [K.lesser[n][l] @ dag(G.retarded[j][l]) for l in range(j+1)]
        G_der_new.lesser[j] += -1j * dt * trapezoid(KxG)

        KxG = [K.retarded[n][l] @ G.lesser[l][j] for l in range(n)]
        G_der_new.lesser[j] += -1j * dt * trapezoid_half_edge(KxG)
        
        G.lesser[n][j] += 0.5 * dt*G_der_new.lesser[j]
        G.lesser[n][j] = np.linalg.solve(idm + 0.5j* dt * h[n] + 0.25j*dt*dt* K.retarded[n][n], G.lesser[n, j])
        
        G_der_new.lesser[j] += -1j*h[n] @ G.lesser[n][j]-0.5j * dt* K.retarded[n][n] @ G.lesser[n][j]
...
def volterra_intdiff(nt, dt, h, K):
    """
     This solves a Volterra integrodifferential equation of
     the second kind,
    ...
    G = NEGF(nt=nt, size=size, sign=1, dt=dt)
    G.retarded[0, 0] = -1j * idm 
    
    if not isinstance(h, list):
        h = [h] * nt 
        
    G_der = Keldysh_derivative(nt=nt, dt=dt)
    G_der.retarded[0] = -1j * h[0] @ G.retarded[0,0]
    
    for n in range(nt):
        G, G_der = volterra_intdiff_timestep(dt, n, h, K, G, G_der)
...
if __name__ == '__main__':
    ...
    sol = KBSolver(h=H, dt=dt, nt=nt, omegac=1)
    ...
    # D =  D0 + D0 @ chi @ D0 # + D0 @ chi @ D0 @ chi @ D0 # convolute(D0, chi)
    # D =  convolute(tmp, D0)
    h = np.zeros((1,1))
    h[0,0] = 1
    ...
    print(D.retarded[3,3])
    fig, ax = plt.subplots()
    # ax.plot(G0.retarded[nt,:, 0, 0].imag)
    t = np.arange(nt+1) * dt
    ax.plot(t, D.retarded[:,0, 0, 0].imag)
    ax.plot(t, D.retarded[:,0, 0, 0].real, label='DR')
    ax.plot(t, D.lesser[:,1, 0, 0].imag, label=r'Im $D^R(t,0)$')
    ax.plot(t, D.lesser[:,1, 0, 0].real, label=r'Re $D^R(t,0)$')
    ax.legend()
