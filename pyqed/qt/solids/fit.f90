...
!     two steps: 1st global linear fitting; 2nd local fitting with basis up to third order 
      subroutine fit(ndim,am,x,p,r,w,u,du,ap,fr)

      use cdat, only: nqt => ntraj

      implicit real*8(a-h, o-z)

      integer*4,intent(in)    :: ndim
      real*8, intent(in), dimension(nqt)  :: w
      real*8, intent(in), dimension(ndim) :: am
      real*8, intent(in), dimension(ndim,nqt) :: x,p,r

      real*8, intent(out), dimension(ndim,nqt) :: du,fr,ap
      real*8, intent(out), dimension(nqt)  :: u
      ...
      real*8 :: f(ndim+1),f2(4),df2(4),ddf2(4),s1(ndim+1,ndim+1),s2(4,4),cp(ndim+1,ndim), & 
                mat(ndim+1,ndim+1),cr(ndim+1,ndim),cpr(4,2),ar(ndim,nqt),               &
