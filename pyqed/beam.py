...
# from pyqed import degrees, eps, mm, np, plt
...
class LaguerreGaussian:
    def __init__(self, w0, kz, omega, pol=[1, 0, 0], l=0, p=0):
        """
    ...
    def width(self, z):
        ...
        return w0 * sqrt(1. + (2. * z/kz/w0**2)**2)

    def field(self, rho, phi, z=0, t=0):
        w0 = self.waist
        wz = self.width(z)
        kz = self.kz 
        l = self.l 
        ...
        x = sqrt(2) * rho / wz 
        zr = kz * w0**2/2 # Rayleigh range
        C = sqrt(2**(abs(l) + 1) * factorial(p)/(pi * factorial(abs(l) + p))) # normalization constant
        mode = C * zr/sqrt(zr**2 + z**2) * x**l * genlaguerre(p, l)(x**2) * \
            exp(-rho**2/wz**2 + 1j * kz * rho**2*z/2./(zr**2 + z**2)) * \
            exp(- 1j * (2*p + abs(l) +1) * np.arctan(z/zr))
        
        return mode * exp(1j * kz * z + 1j * l *  phi) * exp(-1j  * omega * t)
...
beam = LaguerreGaussian(w0=50, kz=10, omega=1.1, p=1, l=1)
...
for i in range(nx):
    for j in range(ny): 
        ...
        f[i, j] = abs(beam.field(rho=r, phi=theta, z=100000))**2

# values = np.random.random((azimuths.size, zeniths.size))
print(f)
ax.matshow(f.real, cmap='coolwarm_r')
