...
def kernel(
    ...
    class AbelianGrowth(NARGBase):
        def __init__(self, table, irrep_table, table_qn, h_irrep, residuals, irrep_residuals, spins):
            super().__init__(
                D=D,
    ...
    growth = AbelianGrowth(
        op_table, irrep_op_table, op_table_qn, Htot_irrep, triple_residuals, irrep_triple_residuals, spin_ops
    )
    ...
    if return_tensors:
        ...
        coeff = np.asarray(X).reshape(d, final_bond_dim, X.shape[1])
        result.append(narg_tensors + [coeff])
    if return_tensor_qns:
        final_bond_dim = narg_tensors[-1].shape[1]
        terminal_total_qn = qn_array(htot_qn).reshape(d, final_bond_dim, -1)
        result.append(
            {
                "factors": tensor_qns,
