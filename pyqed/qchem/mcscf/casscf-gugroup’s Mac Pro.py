...
from .direct_ci import CASCI, build_direct_connectivity, _sigma_compact_derivative_batch_numba
from .orbopt import (
...
class SecondOrderCASSCF(FirstOrderCASSCF):
    ...
    - ``"full"``: matrix-free coupled AH solve over orbital rotations and full
      determinant-space CI response vectors.  State-averaged runs use one
      projected CI-response vector per averaged root.
    """
    ...
    def __init__(
        ...
        factorized_orbital_derivatives="auto",
        max_cycles=None,
        verbose=0,
    ):
        if max_cycles is not None:
        ...
        if self.internal_preopt_hessian not in {"diagonal", "finite_difference"}:
            raise ValueError(
                "internal_preopt_hessian must be 'diagonal' or 'finite_difference'."
            )
        ...
        if isinstance(factorized_orbital_derivatives, str):
            factorized_orbital_derivatives = factorized_orbital_derivatives.lower()
        if factorized_orbital_derivatives not in {"auto", True, False}:
            raise ValueError(
                "factorized_orbital_derivatives must be 'auto', True, or False."
            )
        self.factorized_orbital_derivatives = factorized_orbital_derivatives
    ...
    class _FrozenIntegralRHF:
        def __init__(self, parent_mf, h1_mo, eri_mo, mo_coeff, eri_factors=None):
            ...
            self.eri = None if eri_mo is None else np.array(eri_mo, copy=True)
            self.eri_factors = None if eri_factors is None else np.array(eri_factors, copy=True)
            self.cholesky_jk = self.eri_factors is not None
        ...
        def get_eri_mo(self, mo_coeff=None, notation="chem"):
            ...
            if self.eri is not None:
            ...
            elif self.eri_factors is not None:
                pair_factors = transform_eri_factors_to_mo_pair(
                    self.eri_factors,
                    mo_coeff,
                    mo_coeff,
                ...
                raise ValueError("FrozenIntegralRHF needs dense ERIs or MO pair factors.")
            if notation == "chem":
                return eri
        ...
        def get_veff(self, dm):
            ...
            if self.eri_factors is not None:
                coeff = np.einsum("Prs,rs->P", self.eri_factors, dm, optimize=True)
                j = np.einsum("P,Ppq->pq", coeff, self.eri_factors, optimize=True)
                k = np.einsum(
                    "Ppr,rs,Psq->pq",
                    self.eri_factors,
                    dm,
    ...
    def _assemble_eri_from_pair_factors(self, pair_factors):
        if pair_factors is None:
            return None
        return np.einsum("Ppq,Prs->pqrs", pair_factors, pair_factors, optimize=True)

    def _transform_frozen_pair_factors(self, pair_factors_ref, U):
        if pair_factors_ref is None:
            return None
        return np.einsum(
            "Ppq,pi,qj->Pij",
            pair_factors_ref,
    ...
    def _current_frozen_integrals(self, h1_ref, eri_ref, pair_factors_ref, U):
        h1 = U.conj().T @ h1_ref @ U
        if pair_factors_ref is None:
            _, eri = self._transform_frozen_integrals(h1_ref, eri_ref, U)
            return h1, eri, None
        pair_factors = self._transform_frozen_pair_factors(pair_factors_ref, U)
        eri = self._assemble_eri_from_pair_factors(pair_factors)
        return h1, eri, pair_factors
    ...
    def _make_integral_casci(
        ...
        nstates,
        ci0=None,
        pair_factors_mo=None,
    ):
        frozen_mf = self._FrozenIntegralRHF(
            self.mf,
            h1_mo,
            eri_mo,
            mo_coeff,
            eri_factors=pair_factors_mo,
        )
        ...
        mc.run(
            ...
            method=self.ci_method,
            ci0=ci0,
            use_cholesky=pair_factors_mo is not None and not self.spin_purification,
        )
        self._reorder_tracked_ci_root(mc, requested_nstates, ci0)
        self._update_casci_cache(mc)
        return mc
    ...
    def _micro_line_search(
        ...
        while scale >= 0.125:
            ...
            h1_trial, eri_trial, pair_trial = self._current_frozen_integrals(
                h1_ref,
                eri_ref,
                pair_factors_ref,
                trial_U,
    ...
    def _relaxed_ci_hessian_action(
        ...
        if self.nstates == 1:
            ...
            fock_trial = self._generalized_fock_current(
                h1_trial,
                eri_trial,
                pair_trial,
                trial_mc,
    ...
    def _generalized_fock_current(self, h1_mo, eri_mo, pair_factors_mo, mc, dm1, dm2):
        if pair_factors_mo is not None:
            nocc_like = int(mc.ncore) + int(mc.ncas)
            return generalized_fock_from_factors(
                h1_mo,
                pair_factors_mo[:, :, :nocc_like],
                np.asarray(dm1)[:nocc_like, :nocc_like],
                np.asarray(dm2)[:nocc_like, :nocc_like, :nocc_like, :nocc_like],
            )
        return generalized_fock(h1_mo, eri_mo, dm1, dm2)

    def _use_factorized_orbital_derivatives(self, pair_factors_mo):
        if pair_factors_mo is None:
            return False
        mode = getattr(self, "factorized_orbital_derivatives", "auto")
        if mode is True:
            return True
        if mode is False:
            return False
        naux, nmo, _ = np.asarray(pair_factors_mo).shape
        # The NumPy factor derivative path is memory-light but Python/einsum
        # heavy.  For high-rank molecular Cholesky factors, dense BLAS
        # contractions are still faster until these kernels are compiled.
        return int(naux) <= max(64, int(nmo))

    def _pair_factor_response(self, pair_factors_mo, kappa):
        pair_factors_mo = np.asarray(pair_factors_mo)
        kappa = np.asarray(kappa)
        return (
            np.einsum("Ppj,pi->Pij", pair_factors_mo, kappa, optimize=True)
            + np.einsum("Piq,qj->Pij", pair_factors_mo, kappa, optimize=True)
        )

    def _pair_factor_response_slices(self, pair_factors_mo, kappa, nocc_like):
        pair_factors_mo = np.asarray(pair_factors_mo)
        kappa = np.asarray(kappa)
    ...
    def _generalized_fock_factor_product(
        self,
        left_pair_factors,
        right_pair_factors,
        dm2_occ,
    ):
        left_pair_factors = np.asarray(left_pair_factors)
        right_pair_factors = np.asarray(right_pair_factors)
        dm2_occ = np.asarray(dm2_occ)
        nocc_like = dm2_occ.shape[0]
        contracted = np.einsum(
            "Pst,rqst->Prq",
            right_pair_factors[:, :nocc_like, :nocc_like],
            dm2_occ,
            optimize=True,
        )
        return np.einsum(
            "Ppr,Prq->pq",
            left_pair_factors[:, :, :nocc_like],
            contracted,
            optimize=True,
        )
    ...
    def _orbital_hessian_action_from_factors(
        ...
        kappa = np.asarray(kappa)
        if nocc_like is None:
            nocc_like = np.asarray(dm1).shape[0]
        nocc_like = int(nocc_like)
        dm1_occ = np.asarray(dm1)[:nocc_like, :nocc_like]
        dm2_occ = np.asarray(dm2)[:nocc_like, :nocc_like, :nocc_like, :nocc_like]
        dh1 = h1_mo @ kappa - kappa @ h1_mo
        pair_full_occ = np.asarray(pair_factors_mo)[:, :, :nocc_like]
        ...
        dfock = np.zeros_like(h1_mo, dtype=np.result_type(h1_mo, pair_factors_mo, dm2))
    ...
    def _analytic_orbital_hessian_action(
        ...
        kappa = unpack_nonredundant(vec, mc.ncore, mc.ncas, self.nmo)
        if pair_factors_mo is not None:
            hess_grad = self._orbital_hessian_action_from_factors(
                h1_mo,
                pair_factors_mo,
                dm1,
                dm2,
            ...
            hess_grad = orbital_hessian_action_from_integrals(
                h1_mo,
                eri_mo,
                dm1,
                dm2,
    ...
    def _active_integral_derivative_basis_from_factors(self, mc, h1_mo, pair_factors_mo):
        ncore = int(mc.ncore)
        ncas = int(mc.ncas)
        nocc = ncore + ncas
        core = slice(0, ncore)
        ...
        h1_mo = np.asarray(h1_mo)
        pair_factors_mo = np.asarray(pair_factors_mo)
        b_occ = pair_factors_mo[:, :nocc, :nocc]
        b_active = b_occ[:, ncore:nocc, ncore:nocc]
        dh1_basis = np.zeros((nvar, ncas, ncas), dtype=np.result_type(h1_mo, pair_factors_mo))
        deri_basis = np.zeros((nvar, ncas, ncas, ncas, ncas), dtype=np.result_type(pair_factors_mo))
        de_core = np.zeros(nvar, dtype=float)

        if ncore > 0:
            b_core = b_occ[:, :ncore, :ncore]
            b_core_trace = np.einsum("Pii->P", b_core, optimize=True)
            b_active_core = b_occ[:, ncore:nocc, :ncore]

        for iorb in range(nvar):
            kappa = unpack_nonredundant(eye[:, iorb], mc.ncore, mc.ncas, self.nmo)
            dh1 = h1_mo @ kappa - kappa @ h1_mo
            _, d_occ = self._pair_factor_response_slices(pair_factors_mo, kappa, nocc)
            d_active = d_occ[:, ncore:nocc, ncore:nocc]
            dh1_active = np.array(dh1[active, active], copy=True)
            deri_basis[iorb] = (
                np.einsum("Ppq,Prs->pqrs", d_active, b_active, optimize=True)
                + np.einsum("Ppq,Prs->pqrs", b_active, d_active, optimize=True)
            )
            if ncore > 0:
                d_core = d_occ[:, :ncore, :ncore]
                d_core_trace = np.einsum("Pii->P", d_core, optimize=True)
                d_active_core = d_occ[:, ncore:nocc, :ncore]
                core_j = 2.0 * (
                    np.einsum("Ppq,P->pq", d_active, b_core_trace, optimize=True)
                    + np.einsum("Ppq,P->pq", b_active, d_core_trace, optimize=True)
                )
                core_k = (
                    np.einsum("Ppi,Pqi->pq", d_active_core, b_active_core, optimize=True)
                    + np.einsum("Ppi,Pqi->pq", b_active_core, d_active_core, optimize=True)
                )
                dh1_active = dh1_active + core_j - core_k

                core_coul = (
                    np.dot(d_core_trace, b_core_trace)
                    + np.dot(b_core_trace, d_core_trace)
                )
                core_exch = (
                    np.einsum("Pij,Pji->", d_core, b_core, optimize=True)
                    + np.einsum("Pij,Pji->", b_core, d_core, optimize=True)
                )
                de_core[iorb] = float(
                    np.real(
                        2.0 * np.trace(dh1[core, core])
                        + 2.0 * core_coul
                        - core_exch
                    )
                )
            dh1_basis[iorb] = dh1_active
        return dh1_basis, deri_basis, de_core

    def _active_integral_derivative_basis(self, mc, h1_mo, eri_mo, pair_factors_mo=None):
        key = (
            id(h1_mo),
            id(eri_mo),
            id(pair_factors_mo),
        ...
        if pair_factors_mo is not None:
            dh1_basis, deri_basis, de_core = self._active_integral_derivative_basis_from_factors(
                mc,
                h1_mo,
                pair_factors_mo,
            )
        ...
        eye = np.eye(nvar)
        kappa_basis = np.asarray(
            [
                unpack_nonredundant(eye[:, iorb], mc.ncore, mc.ncas, self.nmo)
                for iorb in range(nvar)
            ]
        )
        h1_mo = np.asarray(h1_mo)
        eri_mo = np.asarray(eri_mo)
        dh1_full = (
            np.einsum("api,pj->aij", kappa_basis, h1_mo, optimize=True)
            + np.einsum("aqj,iq->aij", kappa_basis, h1_mo, optimize=True)
        )
        deri_full = (
            np.einsum("api,pjkl->aijkl", kappa_basis, eri_mo, optimize=True)
            + np.einsum("aqj,iqkl->aijkl", kappa_basis, eri_mo, optimize=True)
            + np.einsum("ark,ijrl->aijkl", kappa_basis, eri_mo, optimize=True)
            + np.einsum("asl,ijks->aijkl", kappa_basis, eri_mo, optimize=True)
        )
        ncore = int(mc.ncore)
        ncas = int(mc.ncas)
        nocc = ncore + ncas
        active = slice(ncore, nocc)
        dh1_basis = np.array(dh1_full[:, active, active], copy=True)
        if ncore > 0:
            core = slice(0, ncore)
            core_j = 2.0 * np.einsum(
                "apqii->apq",
                deri_full[:, active, active, core, core],
                optimize=True,
            )
            core_k = np.einsum(
                "apiqi->apq",
                deri_full[:, active, core, active, core],
                optimize=True,
            )
            dh1_basis = dh1_basis + core_j - core_k
        deri_basis = np.array(deri_full[:, active, active, active, active], copy=True)
        self._full_derivative_cache = {
            "key": key,
            "dh1": dh1_basis,
            "deri": deri_basis,
    ...
    def _core_energy_derivative_basis(self, mc, h1_mo, eri_mo, pair_factors_mo=None):
        key_base = (
            id(h1_mo),
            id(eri_mo),
            id(pair_factors_mo),
        ...
        if pair_factors_mo is not None:
            self._active_integral_derivative_basis(
                mc,
                h1_mo,
                eri_mo,
                pair_factors_mo=pair_factors_mo,
            )
            if (
                self._full_derivative_cache is not None
                and self._full_derivative_cache.get("core_key") == key
        ...
        if (
            self._full_derivative_cache is not None
            and self._full_derivative_cache.get("key")
            == key_base
            and "dh1_full" in self._full_derivative_cache
            and "deri_full" in self._full_derivative_cache
        ):
            dh1_full = self._full_derivative_cache["dh1_full"]
            deri_full = self._full_derivative_cache["deri_full"]
            ncore = int(mc.ncore)
            if ncore <= 0:
                vals = np.zeros(dh1_full.shape[0], dtype=float)
            else:
                core = slice(0, ncore)
                vals = 2.0 * np.einsum(
                    "aii->a",
                ...
                vals = vals + 2.0 * np.einsum(
                    "aiijj->a",
                    deri_full[:, core, core, core, core],
                    optimize=True,
                )
                vals = vals - np.einsum(
                    "aijji->a",
    ...
    def _derivative_sigma_basis(self, mc, h1_mo, eri_mo, c0, pair_factors_mo=None):
        dh1_basis, deri_basis = self._active_integral_derivative_basis(
            mc,
            h1_mo,
            eri_mo,
            pair_factors_mo=pair_factors_mo,
        ...
        try:
            sigma_basis = _sigma_compact_derivative_batch_numba(
    ...
    def _exact_orbital_gradient_vector(self, mc, h1_mo, eri_mo, ci, pair_factors_mo=None):
        """
        ...
        sigma_basis = self._derivative_sigma_basis(
            mc,
            h1_mo,
            eri_mo,
            ci,
        ...
        grad = sigma_basis @ ci
        grad = grad + self._core_energy_derivative_basis(
            mc,
            h1_mo,
            eri_mo,
            pair_factors_mo=pair_factors_mo,
    ...
    def _ci_gradient_from_orbital_response(
        self,
        mc,
        h1_mo,
        eri_mo,
    ...
    def _orbital_gradient_from_ci_response_adjoint(
        self,
        mc,
        h1_mo,
        eri_mo,
    ...
    def _full_coupled_step(
        ...
        """
        if self.nstates != 1 or self.weights is not None:
            return self._full_state_averaged_coupled_step(
                mc,
                h1_mo,
                eri_mo,
                grad_vec,
        ...
        sigma_basis = np.asarray(
            self._derivative_sigma_basis(
                mc,
                h1_mo,
                eri_mo,
                c0,
        ...
        coupling_block = sigma_basis - np.outer(sigma_basis @ c0, c0)
        ...
        def matvec(vec):
            ...
            out_orb += coupling_block @ ci_part
            out_ci = mc.ci_sigma(ci_part) - active_energy * ci_part
            out_ci += coupling_block.T @ orb_part
    ...
    def _full_state_averaged_coupled_step(
        self,
        mc,
        h1_mo,
        eri_mo,
        ...
        """
        Matrix-free coupled AH step for state-averaged full CI response.

        The response variables are one determinant-space vector per averaged
        root, projected outside the optimized-state subspace so rotations among
        averaged roots remain redundant.
        """
        nroots = max(1, int(self.nstates))
        if len(mc.ci) < nroots:
            return np.asarray(fallback_step, dtype=float), self._copy_ci_guess(mc.ci)
        weights = self.weights
        if weights is None:
            weights = np.ones(nroots, dtype=float) / float(nroots)
        weights = np.asarray(weights, dtype=float)

        roots = np.column_stack([np.asarray(mc.ci[i], dtype=float) for i in range(nroots)])
        roots, _ = np.linalg.qr(roots)
        ndet = roots.shape[0]
        n_orb = grad_vec.size
        if ndet == 0 or n_orb == 0:
            return np.asarray(fallback_step, dtype=float), [roots[:, i].copy() for i in range(nroots)]
        ...
        for root in range(nroots):
            scale = max(2.0 * float(weights[root]), self.level_shift)
            ci_blocks.append(
                np.maximum(np.abs(scale * (ci_diag - active_energies[root])), self.level_shift)
            )
        precond_diag = np.concatenate((orb_hdiag, *ci_blocks))
        total_grad = np.concatenate(
            (np.asarray(grad_vec, dtype=float), np.zeros(nroots * ndet, dtype=float))
        )
        ...
        for root in range(nroots):
            ...
            couplings.append(sigma_basis - (sigma_basis @ roots) @ roots.T)

        def project_ci(vec):
            vec = np.asarray(vec, dtype=float).copy()
            vec -= roots @ (roots.T @ vec)
            return vec

        def split(vec):
            vec = np.asarray(vec, dtype=float)
            ci_mat = vec[n_orb:].reshape(nroots, ndet)
            ci_mat = np.asarray([project_ci(ci_mat[root]) for root in range(nroots)])
            return vec[:n_orb], ci_mat

        def join(orb, ci_mat):
            return np.concatenate((np.asarray(orb, dtype=float), np.asarray(ci_mat).reshape(-1)))

        def matvec(vec):
            orb_part, ci_parts = split(vec)
            out_orb = np.asarray(hessian_action(orb_part), dtype=float)
            out_ci = np.zeros_like(ci_parts)
            for root in range(nroots):
                scale = 2.0 * float(weights[root])
                coupling = couplings[root]
                ci_part = ci_parts[root]
                out_orb += scale * (coupling @ ci_part)
                out_ci[root] = scale * (
                    mc.ci_sigma(ci_part) - active_energies[root] * ci_part
                    + coupling.T @ orb_part
                )
                out_ci[root] = project_ci(out_ci[root])
            return join(out_orb, out_ci)

        seed = join(np.asarray(fallback_step, dtype=float), np.zeros((nroots, ndet)))
        diag_step = -total_grad / precond_diag
        seeds = []
        for col in (seed, diag_step):
            norm = np.linalg.norm(col)
        ...
        ci_guess_mat = roots + ci_steps.T
        ci_guess_mat, _ = np.linalg.qr(ci_guess_mat)
        return np.asarray(orb_step, dtype=float), [
            ci_guess_mat[:, i].copy() for i in range(nroots)
        ]
    ...
    def _internal_preopt_evaluate(self, mo_coeff, ci_guess):
        mc = self._make_casci(mo_coeff, nstates=self.nstates, ci0=ci_guess)
        energy = self._objective_energy(mc, self.state_id)
        h1_mo = self.mf.get_hcore_mo(mo_coeff)
        eri_mo = self.mf.get_eri_mo(mo_coeff, notation="chem")
        dm1, dm2 = self._effective_rdms(mc, self.state_id)
    ...
    def _internal_preopt_step(self, mo_coeff, ci_guess, macro):
        """
        ...
        mask = self._core_active_mask(mc.ncore, mc.ncas, self.nmo)
        ...
        if self.internal_preopt_hessian == "finite_difference":
    ...
    def _internal_preopt(self, mo_coeff, ci_guess, macro):
        if self.internal_preopt_steps <= 0:
            return mo_coeff, ci_guess
        local_ci = self._copy_ci_guess(ci_guess)
        for _ in range(self.internal_preopt_steps):
    ...
    def _restart_solver(self):
        trial = SecondOrderCASSCF(
            ...
            factorized_orbital_derivatives=self.factorized_orbital_derivatives,
    ...
    def _try_active_restarts(self, initial_mo_coeff, best_energy):
        ...
        if self.history:
            ...
            if (self.converged and self.coupling == "full") or last_grad <= self.conv_tol_grad:
                return
    ...
    def run(
        ...
        for macro in range(1, self.max_cycle + 1):
            ...
            pair_factors_ref = None
            if self.use_cholesky_integrals and not self.spin_purification:
                pair_factors_ref = transform_eri_factors_to_mo_pair(
                    _get_mf_cholesky_factors(self.mf),
                    mo_coeff,
                    mo_coeff,
                )
            ...
            for micro in range(1, self.max_micro_cycle + 1):
                h1_cur, eri_cur, pair_factors_cur = self._current_frozen_integrals(
                    h1_ref,
                    eri_ref,
                    pair_factors_ref,
                    U,
                )
                derivative_pair_factors_cur = (
                    pair_factors_cur
                    if self._use_factorized_orbital_derivatives(pair_factors_cur)
                    else None
                )
                ...
                mc = self._make_integral_casci(
                    ...
                    solve_nstates,
                    ci0=local_ci_guess,
                    pair_factors_mo=pair_factors_cur,
                )
                energy = self._objective_energy(mc, self.state_id)
                dm1, dm2 = self._effective_rdms(mc, self.state_id)
                fock = self._generalized_fock_current(
                    h1_cur,
                    eri_cur,
                    pair_factors_cur,
                    mc,
                ...
                if self.nstates == 1:
                    grad_vec = self._exact_orbital_gradient_vector(
                        ...
                        eri_cur,
                        mc.ci[self.state_id],
                        pair_factors_mo=derivative_pair_factors_cur,
                    )
                ...
                if self.coupling == "qn" and qn_base_hessian_action is None:
                    ...
                    pair_qn = (
                        None
                        if derivative_pair_factors_cur is None
                        else np.array(derivative_pair_factors_cur, copy=True)
                    )
                ...
                micro_record = {
                    ...
                    "cholesky_integrals": bool(pair_factors_cur is not None),
                }
                ...
                if np.dot(step_vec, grad_vec) >= 0.0:
                    step_vec = diag_step
                if use_norm_trust:
                    step_vec = limit_step_trust_radius(step_vec, step_trust_radius)
                    unpack_step_limit = None
                else:
                    step_vec = limit_step_norm(step_vec, step_limit)
                    unpack_step_limit = step_limit
                step_hv = np.asarray(hessian_action(step_vec), dtype=float)
                predicted_reduction = -float(
                    np.dot(grad_vec, step_vec) + 0.5 * np.dot(step_vec, step_hv)
                )
                micro_record["ah_predicted_reduction"] = float(predicted_reduction)
                kappa = unpack_nonredundant(
                    step_vec,
                    mc.ncore,
                    mc.ncas,
                    self.nmo,
                    max_step=unpack_step_limit,
                )
                ...
                if not accepted:
                    ...
                    break
                actual_reduction = float(energy - self._objective_energy(trial_mc, self.state_id))
                scaled_predicted = predicted_reduction * float(accepted_scale) ** 2
                if scaled_predicted <= 1.0e-12:
                    ah_ratio = np.inf if actual_reduction > 0.0 else -np.inf
                else:
                    ah_ratio = actual_reduction / scaled_predicted
                micro_record["ah_actual_reduction"] = float(actual_reduction)
                micro_record["ah_ratio"] = float(ah_ratio)
                if self.ah_adaptive_trust:
