...
def kernel(mc, U0, nelecas, ncas, C0, h1e, eri, max_cycles=30, tol=1e-6,
    ...
    while k < max_cycles:
        ...
        if getattr(mc, "verbose", 0) >= 1:
            print(
                "CASSCF macro {:3d}  E = {:.12f}  dE = {:.3e}".format(
                    k,
                    float(np.real(current_mc.e_tot)),
                    float(np.real(current_mc.e_tot - e_old)),
                )
            )
...
def kernel_state_average(mc, weights, U0, nelecas, ncas, C0, h1e, eri,
    ...
    while k < max_cycles:
        ...
        if getattr(mc, "verbose", 0) >= 1:
            print(
                "CASSCF macro {:3d}  E(state-avg) = {:.12f}  dE = {:.3e}  roots = {}".format(
                    k,
                    float(np.real(eAve)),
                    float(np.real(eAve - e_old)),
                    np.array2string(np.asarray(current_mc.e_tot), precision=12),
                )
            )
