...
class IR:
    ...
    def __init__(
        ...
        self.reuse_scf_guess = bool(reuse_scf_guess)
    ...
    def _finite_difference_energy_hessian(self, method, step):
        ...
        cache = {}
        scanner = None
        if self.reuse_scf_guess and hasattr(method, "as_scanner"):
            scanner = method.as_scanner(build_driver=getattr(method.mol, "_build_driver", None))

        def energy_at(flat):
            key = tuple(np.round(np.asarray(flat, dtype=float), 12))
            if key not in cache:
                coords = np.asarray(flat).reshape(natom, 3)
                if scanner is None:
                    step_method = self._build_method_at_coords(method, coords)
                    cache[key] = float(step_method.e_tot)
                else:
                    cache[key] = float(scanner(coords))
            return cache[key]
        ...
        try:
            ...
            method.mol.set_geom(coords0)
            driver = getattr(method.mol, "_build_driver", None) or "builtin"
            if driver == "builtin":
                method.mol.build(driver=driver, options=getattr(method.mol, "builtin_options", None))
            else:
                method.mol.build(driver=driver)
        return hess
    ...
    def _build_method_at_coords(self, method, coords):
        ...
        if name == "rhf":
            ...
            if hasattr(method, "max_cycle"):
                ...
                run_kwargs["max_cycle"] = method.max_cycle
