...
def hartree_fock(mol, dm0=None, init_guess='hcore', max_cycle=50, tol=1e-8):
    ...
    # S = mol.overlap
    S = np.array(mol.overlap, dtype=np.complex128)
    # print('S',S)
    ...
    # hcore = mol.hcore
    hcore = np.array(mol.hcore, dtype=np.complex128)
...
#                    two_electron[mu,lamb,sigma,v] = exchange
    ...
    diis_error_matrices = np.zeros((maxdiis, nbas, nbas), dtype=np.complex128)
    diis_fock_matrices = np.zeros((maxdiis, nbas, nbas), dtype=np.complex128)
    ...
    def diis(fock, dens, overlap, orth, iter):
        ...
        """
        # is_complex = np.iscomplexobj(fock)
        # dtype = np.complex128 if is_complex else np.float64
        
        diis_fock = np.zeros_like(fock, dtype=np.complex128)
        ...
        for k in reversed(range(1, min(iter, maxdiis))):
            ...
            diis_error_matrices[k] = diis_error_matrices[k-1].copy()
            diis_fock_matrices[k] = diis_fock_matrices[k-1].copy()

        # calculate error matrix
        error_mat = reduce(np.dot, (fock, dens, overlap))
        error_mat -= error_mat.conj().T

        # put orthogonal error matrix in storage
        # pulay use S^(-1/2) but here we choose whatever the user has defined
        diis_error_matrices[0]  = reduce(np.dot, (orth.conj().T, error_mat, orth))
        diis_fock_matrices[0] = fock.copy()
        diis_error_index = np.abs(diis_error_matrices[0]).argmax()
        diis_error = np.abs(np.ravel(diis_error_matrices[0])[diis_error_index])

        # calculate B-matrix and solve for coefficients that reduces error
        bsize = min(iter, maxdiis)-1
        bmat = -1.0 * np.ones((bsize+1,bsize+1), dtype=np.complex128)
        rhs = np.zeros(bsize+1, dtype=np.complex128)
        bmat[bsize, bsize] = 0
        rhs[bsize] = -1
        for b1 in range(bsize):
            for b2 in range(bsize):
                bmat[b1, b2] = np.trace(diis_error_matrices[b1].conj().T @ diis_error_matrices[b2])
    ...
    for scf_iter in range(max_cycle):
        ...
        # F_hermitian = Fprime.conj().T
        # print('hermitian', Fprime.all() == F_hermitian.all())
        #println("F': $Fprime")
        mo_energy, Cprime = eigh(Fprime)
        # print("Cprime is orthogonal", Cprime.conj().T @ Cprime)
        # print("epsilon: ", epsilon)
        #print("C': ", Cprime)
        mo_coeff = np.dot(X,Cprime)
        # Cprimet = np.linalg.solve(X, mo_coeff)
        # Cprimet = np.linalg.inv(X) @ mo_coeff
        # print("Cprime is orthogonal", Cprimet.conj().T @ Cprimet)
        # print("C: ", C)
        ...
        # print('iter energy', total_energy)
...
if __name__ == '__main__':
    ...
    mol = Molecule(atom='Li 0.6 2 0; H 0 0 1.4', unit='b', basis='sto3g')
    ...
    C0 = hf.mo_coeff
    s, U = eigh(mol.overlap)

    # building transformation matrix S^{-1/2}
    X = U.dot(np.diagflat(s**(-0.5)).dot(dagger(U)))
    C0_p = np.linalg.inv(X) @ C0
    print('onerlap', C0_p.conj().T @ C0_p)
    # # compare hcore
    # h1 = mol.hcore
    # h2 = hf.hcore
    # h3 = hf.get_hcore()
    # print('hcore===', h1.all() == h3.all())
