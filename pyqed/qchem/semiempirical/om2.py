...
from pyqed.units import angstrom2au, electronvolt


EV_TO_HARTREE = electronvolt
...
def _build_cross_overlap_matrix(
    left_orbitals,
    left_params,
    left_coords,
    right_orbitals,
    ...
    """Build the approximate valence-AO overlap between two geometries."""
    output = np.empty((len(left_orbitals), len(right_orbitals)), dtype=float)
    for p, (orb_p, params_p) in enumerate(zip(left_orbitals, left_params)):
        for q, (orb_q, params_q) in enumerate(zip(right_orbitals, right_params)):
            displacement = (
                np.asarray(right_coords[orb_q.atom_index])
                - np.asarray(left_coords[orb_p.atom_index])
            )
            output[p, q] = _pair_overlap(
                params_p, params_q, orb_p, orb_q, displacement
            )
    return output
...
def _symmetric_inverse_sqrt(matrix, threshold=1e-10):
    values, vectors = _safe_eigh(matrix)
    if values[0] <= threshold:
        raise np.linalg.LinAlgError(
            f"AO overlap is not positive definite (minimum eigenvalue {values[0]:.3e})."
        )
    return (vectors / np.sqrt(values)) @ vectors.T


def _pulay_fock(focks, errors, regularization=1e-12):
    """Return the Pulay-extrapolated Fock matrix, or the newest matrix."""
    count = len(focks)
    if count < 2:
        return focks[-1]
    system = np.empty((count + 1, count + 1), dtype=float)
    system[:count, :count] = [
        [float(np.vdot(left, right).real) for right in errors]
        for left in errors
    ]
    ...
    rhs[count] = -1.0
    system[:count, :count] += regularization * np.eye(count)
    try:
        coefficients = np.linalg.solve(system, rhs)[:count]
    except np.linalg.LinAlgError:
        return focks[-1]
    output = sum(weight * fock for weight, fock in zip(coefficients, focks))
...
class OM2Reference:
    ...
    def get_ao_cross_overlap(self, other, orthogonalized=True):
        """Return valence-AO transport between two native OM2 geometries."""
        if not isinstance(other, OM2Reference):
            raise TypeError("OM2 cross-overlap requires another OM2Reference.")
        left_orbitals = self.om2.valence_orbitals()
        right_orbitals = other.om2.valence_orbitals()
        if tuple(orb.label for orb in left_orbitals) != tuple(
            orb.label for orb in right_orbitals
        ):
            raise ValueError("OM2 cross-overlap requires matching atom and AO order.")
        left_params = tuple(
            self.om2.parameters.for_symbol(orb.symbol) for orb in left_orbitals
        )
        right_params = tuple(
            other.om2.parameters.for_symbol(orb.symbol) for orb in right_orbitals
        )
        cross = _build_cross_overlap_matrix(
            left_orbitals,
            left_params,
            self.om2.atom_coords(),
            right_orbitals,
            right_params,
            other.om2.atom_coords(),
        )
        if not orthogonalized:
            return cross
        left_metric = _build_overlap_matrix(
            left_orbitals, left_params, self.om2.atom_coords()
        )
        right_metric = _build_overlap_matrix(
            right_orbitals, right_params, other.om2.atom_coords()
        )
        return (
            _symmetric_inverse_sqrt(left_metric)
            @ cross
            @ _symmetric_inverse_sqrt(right_metric)
        )

    def get_mo_cross_overlap(self, other):
        """Return transported MO overlap for MRCI state overlaps."""
        if self.mo_coeff is None or other.mo_coeff is None:
            raise ValueError("Run both OM2 references before computing MO overlap.")
        return self.mo_coeff.T @ self.get_ao_cross_overlap(other) @ other.mo_coeff
    ...
    def _run_diis(self, dm, max_cycle, conv_tol, damping, diis_space):
        density_tolerance = np.sqrt(conv_tol)
        focks = []
        errors = []
        last_energy = None
        converged = False
        ...
        for cycle in range(1, max_cycle + 1):
            ...
            error = fock @ dm - dm @ fock
            focks.append(fock)
            errors.append(error)
            focks = focks[-diis_space:]
            errors = errors[-diis_space:]
            effective_fock = _pulay_fock(focks, errors)
            eps, coeff = _safe_eigh(effective_fock)
            new_dm = _density_from_coeff(coeff, np.count_nonzero(self.mo_occ))
            if cycle > 1 and damping:
                new_dm = (1.0 - damping) * new_dm + damping * dm
            physical_fock = _build_fock(self.hcore, self.eri, new_dm)
            energy = (
                _electronic_energy(self.hcore, physical_fock, new_dm) + self.enuc
            )
            residual = np.linalg.norm(physical_fock @ new_dm - new_dm @ physical_fock)
            delta_density = np.linalg.norm(new_dm - dm)
            delta_energy = np.inf if last_energy is None else abs(energy - last_energy)
            dm = new_dm
            last_energy = energy
            if (
                delta_energy < conv_tol
                and delta_density < density_tolerance
                and residual < density_tolerance
            ):
                converged = True
                break
        return converged, cycle, energy, residual, dm
    ...
    def run(
        ...
        if diis_space < 2:
            raise ValueError("diis_space must be at least two.")
        if n_starts < 1:
            raise ValueError("n_starts must be positive.")
        self.mo_occ = np.zeros(self.hcore.shape[0])
        self.mo_occ[:nocc] = 2

        _, core_coeff = _safe_eigh(self.hcore)
        guesses = [_density_from_coeff(core_coeff, nocc)]
        rng = np.random.default_rng(random_seed)
        for _ in range(n_starts - 1):
            orbitals, _ = np.linalg.qr(rng.normal(size=self.hcore.shape))
            guesses.append(_density_from_coeff(orbitals, nocc))

        solutions = []
        for guess in guesses:
            result = self._run_diis(
                guess, int(max_cycle), float(conv_tol), float(damping), int(diis_space)
            )
            if result[0]:
                solutions.append(result)
        self.scf_solutions = tuple(
            (float(result[2]), int(result[1]), float(result[3]))
            for result in solutions
        )
        if solutions:
            converged, cycle, _, _, dm = min(solutions, key=lambda result: result[2])
            self.converged = converged
        else:
            converged, cycle, _, _, dm = self._run_diis(
                guesses[0], int(max_cycle), float(conv_tol), float(damping), int(diis_space)
            )
            self.converged = converged

        # Canonicalize against the physical Fock matrix of the selected density.
        fock = _build_fock(self.hcore, self.eri, dm)
        eps, coeff = _safe_eigh(fock)
        canonical_dm = _density_from_coeff(coeff, nocc)
        canonical_fock = _build_fock(self.hcore, self.eri, canonical_dm)
        self.niter = cycle
        self.mo_coeff = coeff
        self.mo_energy = eps
        self.dm = canonical_dm
        self.residual_norm = float(
            np.linalg.norm(canonical_fock @ canonical_dm - canonical_dm @ canonical_fock)
        )
        self.e_elec = _electronic_energy(self.hcore, canonical_fock, canonical_dm)
