...
class DMRG(CASCI):
    ...
    def build(self, mo_coeff=None, *, build_mpo=True):
        ...
        if not build_mpo:
            ...
            if self._active_integral_build_info is not None:
                self._active_integral_build_info["representation"] = "integrals_only"
            return self
        ...
        **kwargs,
        ...
        if self.sym_mgr.enabled:
            if getattr(self.sym_mgr, "has_nonabelian", False):
                ...
                if (
                    mo_coeff is not None
                    or getattr(self, "h1e", None) is None
