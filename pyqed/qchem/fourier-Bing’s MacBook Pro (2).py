...
try:
    from pyqed.qchem import _gdf_cpp
except Exception:  # pragma: no cover - optional C++ acceleration
    _gdf_cpp = None
...
def has_periodic_pair_ft_backend():
    """Return whether periodic AO-pair Fourier sums have a compiled backend."""
    return (
        _ao_block_pair_ft_primitive_sum_numba is not None
        or (
            _gdf_cpp is not None
            and hasattr(_gdf_cpp, "periodic_pair_ft_primitive_sum")
        )
    )


def has_periodic_pair_ft_many_backend():
    """Return whether batched periodic AO-pair Fourier sums are available."""
    return (
        _gdf_cpp is not None
        and hasattr(_gdf_cpp, "periodic_pair_ft_primitive_sum_many")
    )


def has_periodic_pair_ft_contract_backend():
    """Return whether contracted batched periodic AO-pair Fourier sums are available."""
    return (
        _gdf_cpp is not None
        and hasattr(_gdf_cpp, "periodic_pair_ft_primitive_contract_many")
    )


def _prefer_cpp_gdf_pair_ft():
    value = os.environ.get("PYQED_GDF_CPP", "")
    return value.strip().lower() in {"1", "true", "yes", "on", "cpp"}
...
class AOBlockPairFTPlan:
    ...
    def periodic_sum(
        ...
        if (
            ...
            and _gdf_cpp is not None
            and hasattr(_gdf_cpp, "periodic_pair_ft_primitive_sum")
            and (
                _prefer_cpp_gdf_pair_ft()
                or _ao_block_pair_ft_primitive_sum_numba is None
            )
        ):
            return _gdf_cpp.periodic_pair_ft_primitive_sum(
                gvecs,
                self.pair_p,
                self.pair_q,
                self.nleft,
    ...
    def periodic_sum_many(
        ...
        if phases.ndim != 2 or phases.shape[1] != right_origins_batch.shape[0]:
        ...
        if (
            ...
            return _gdf_cpp.periodic_pair_ft_primitive_sum_many(
                gvecs,
                self.pair_p,
                self.pair_q,
                self.nleft,
    ...
    def periodic_contract_many(
        ...
        if (
            ...
            return _gdf_cpp.periodic_pair_ft_primitive_contract_many(
                gvecs,
                self.pair_p,
                self.pair_q,
                self.nleft,
        ...
        pair_ft = self.periodic_sum_many(
            gvecs,
            left_origins=left_origins,
            right_origins_batch=right_origins_batch,
            phases=phases,
        ...
        return np.einsum("ga,xgmn->xamn", weighted_aux, pair_ft, optimize=True)
