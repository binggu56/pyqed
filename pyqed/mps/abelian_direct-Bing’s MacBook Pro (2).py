...
class AbelianPackedBoundaryTensor:
    ...
    __slots__ = ("keys", "blocks", "dirs", "qns", "source", "_data", "_signature")
...
def abelian_layout_axis_sector_dims(layout, axis, *, qns=None):
    dims = {}
    axis = int(axis)
    if qns is not None:
        try:
            for qn in tuple(qns)[axis]:
                dims.setdefault(qn, 0)
        except Exception:
            pass
    for key, shape in tuple(layout or ()):
        key = tuple(key)
        shape = tuple(int(dim) for dim in shape)
        if len(key) <= axis or len(shape) <= axis:
            continue
        dims[key[axis]] = max(int(dims.get(key[axis], 0)), int(shape[axis]))
    return dims
...
def abelian_merge_layout_axis_sector_dims(dims, layout, axis, *, qns=None):
    for qn, dim in abelian_layout_axis_sector_dims(layout, axis, qns=qns).items():
        if int(dim) > 0:
            dims[qn] = max(int(dims.get(qn, 0)), int(dim))
...
def abelian_safe_two_site_layout_map_from_layout(layout, qns=None, w_sites=()):
    """Layout-only equivalent of :func:`abelian_safe_two_site_layout_map`."""

    layout = tuple(
        (tuple(key), tuple(int(dim) for dim in shape))
        for key, shape in tuple(layout or ())
    )
    if not layout or len(layout[0][0]) != 4:
        return None
    left_dims = abelian_layout_axis_sector_dims(layout, 0, qns=qns)
    right_dims = abelian_layout_axis_sector_dims(layout, 1, qns=qns)
    phys_left_dims = abelian_layout_axis_sector_dims(layout, 2, qns=qns)
    phys_right_dims = abelian_layout_axis_sector_dims(layout, 3, qns=qns)
    w_sites = tuple(w_sites or ())
    if len(w_sites) >= 2:
        abelian_merge_axis_sector_dims(phys_left_dims, w_sites[0], 2)
        abelian_merge_axis_sector_dims(phys_left_dims, w_sites[0], 3)
    ...
    *,
    ...
    if native_split is not None:
        try:
            ...
            _ABELIAN_SVD_KERNEL_STATS["cpp_flat_split_update_calls"] += 1
            _ABELIAN_SVD_KERNEL_STATS["cpp_flat_split_update_blocks"] += int(len(layout or ()))
            if native_stats:
                _ABELIAN_SVD_KERNEL_STATS["cpp_flat_split_update_sectors"] += int(
                    native_stats.get("sectors", 0)
                )
                _ABELIAN_SVD_KERNEL_STATS["cpp_flat_split_update_rows"] += int(
                    native_stats.get("rows", 0)
                )
                _ABELIAN_SVD_KERNEL_STATS["cpp_flat_split_update_cols"] += int(
                    native_stats.get("cols", 0)
                )
                _ABELIAN_SVD_KERNEL_STATS["cpp_flat_split_update_dim"] += int(
...
class AbelianContextualDirectFamilyBuilder:
    ...
    def _batch_build_missing_cached(
        ...
        (
            ...
            debug_stats=self.stats.setdefault("contextual_route_lazy_pack", {"calls": 0}),
            debug_side=str(side),
        )
    ...
    def precompute_boundaries(self, family_name, records):
        ...
        left_payload_counts = _contextual_boundary_payload_counts(left_values)
        right_payload_counts = _contextual_boundary_payload_counts(right_values)
        ...
        return AbelianContextualBoundaryBatch(
            left_results,
            right_results,
            tuple(left_values),
            tuple(right_values),
        ...
        *,
        ...
        if route_plan is not None:
            ...
            use_precomputed_arrays = bool(
                ...
                and len(left_values) == len(left_keys)
