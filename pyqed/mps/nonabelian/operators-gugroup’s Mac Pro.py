...
class ReducedTensorOperator:
        ...
        for (q_out, q_in), value in dict(self.reduced_blocks).items():
            ...
            if out_dim not in {1, out_irrep.dim}:
                raise NotImplementedError(
                    "ReducedTensorOperator currently requires either explicit irrep "
                    "physical dimensions or degeneracy-only output sectors."
                )
            if in_dim not in {1, in_irrep.dim}:
                raise NotImplementedError(
                    "ReducedTensorOperator currently requires either explicit irrep "
                    "physical dimensions or degeneracy-only input sectors."
                )
    ...
    def component_block(self, two_m_component, q_out, q_in):
        ...
        if self.phys_out_leg.dim(q_out) == 1 and self.phys_in_leg.dim(q_in) == 1:
            # Degeneracy-only physical legs store Wigner-Eckart amplitudes, not
            # explicit spin components.  Expanding one spherical component
            # therefore carries the usual tensor-rank normalization; explicit
            # physical legs below get their component weights from CG tables.
            component_norm = np.sqrt(float(self.rank_irrep.dim))
            return np.asarray([[phase * reduced_value / component_norm]], dtype=self.dtype)
...
def reduced_physical_leg_from_spatial_orbital(site=None):
    """
    Build the degeneracy-only physical leg for one SU(2)-adapted spatial orbital.

    Unlike :func:`physical_leg_from_spatial_orbital`, each local charge-spin
    sector has degeneracy dimension one.  The SU(2) irrep dimension is then
    carried by fusion/CG maps rather than by explicit physical block axes,
    matching TensorKit's representation-space/degeneracy-space split.
    """
    reduced_leg = _reduced_spatial_leg()
    if site is None or isinstance(site, SpatialOrbitalSite):
        return reduced_leg
    if isinstance(site, PhysicalLeg):
        if site != reduced_leg:
            raise ValueError(
                "reduced_physical_leg_from_spatial_orbital expects the degeneracy-only spatial leg."
            )
        return site
    if isinstance(site, NonabelianTensor):
        if site.rank != 3:
            raise ValueError(
                "reduced_physical_leg_from_spatial_orbital expects a rank-3 NonabelianTensor site tensor."
            )
        site = normalize_site_tensor_layout(site)
        dims = {}
        for key, block in site.data.items():
            ...
            if prev != dim:
                raise ValueError(
                    f"Inconsistent physical degeneracy for sector {sector!r}: {prev} vs {dim}."
                )
        leg = PhysicalLeg.from_dims(dims, sectors=tuple(dict.fromkeys(site.qns[1])))
        if leg != reduced_leg:
            raise ValueError(
                "reduced_physical_leg_from_spatial_orbital expects a degeneracy-only spatial site tensor."
            )
        return leg
    raise TypeError(
        "reduced_physical_leg_from_spatial_orbital expects None, SpatialOrbitalSite, PhysicalLeg, "
        "or a rank-3 NonabelianTensor."
    )
...
def reduced_spatial_identity(site=None, *, dtype=float):
    return identity_operator(reduced_physical_leg_from_spatial_orbital(site), dtype=dtype)
...
def reduced_spatial_number(site=None, *, dtype=float):
    phys_leg = reduced_physical_leg_from_spatial_orbital(site)
    weights = {sector: float(sector.charge) for sector in phys_leg.sectors}
    return _projector_weights(weights, phys_leg=phys_leg, dtype=dtype)
...
def reduced_spatial_double_occupancy(site=None, *, dtype=float):
    phys_leg = reduced_physical_leg_from_spatial_orbital(site)
    weights = {sector: float(sector.charge == 2) for sector in phys_leg.sectors}
    return _projector_weights(weights, phys_leg=phys_leg, dtype=dtype)
...
def reduced_spatial_parity(site=None, *, dtype=float):
    """
    Fermionic parity ``(-1)^N`` on a degeneracy-only spatial site.
    """
    phys_leg = reduced_physical_leg_from_spatial_orbital(site)
    weights = {sector: float((-1) ** sector.charge) for sector in phys_leg.sectors}
    return _projector_weights(weights, phys_leg=phys_leg, dtype=dtype)
