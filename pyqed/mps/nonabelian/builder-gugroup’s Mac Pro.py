...
def _site_operator_signature(operator):
    """
    Hash a SiteOperator by exact stored block content for prefix sharing.
    """
    parts = []
    for (q_out, q_in), block in sorted(
        operator.blocks.items(),
...
class AutoMPO:
    ...
    def add_fermionic_string(
        ...
        *site_operators,
        ...
        """
        Add a product of global fermionic operators with Jordan-Wigner strings.

        ``site_operators`` are supplied in the physical operator order, e.g.
        ``(p, cdag_p), (r, cdag_r), (s, c_s), (q, c_q)``.  Each operator is
        expanded as ``prod_{k < site} P_k * op_site`` and same-site factors are
        composed in that order before the final product term is inserted.
        """
        if not site_operators:
            raise ValueError("add_fermionic_string requires at least one (site, operator) pair.")

        normalized = []
        dtypes = [np.asarray(coeff).dtype, float]
        for item in site_operators:
            if not isinstance(item, tuple) or len(item) != 2:
                raise TypeError(
                    "add_fermionic_string expects arguments like (site, operator)."
                )
            site, operator = item
            site = int(site)
            if site < 0 or site >= self.nsites:
                raise IndexError(
                    f"Fermionic string site {site} out of range for {self.nsites} sites."
                )
            _validate_site_operator(
                operator,
                self.site_legs[site],
                label="add_fermionic_string",
            )
            normalized.append((site, operator))
            dtypes.append(operator.dtype)
        ...
        def parity_for(site):
            site = int(site)
            if site not in parity_cache:
                if parity_operator is None:
                    parity = _parity_operator(self.site_legs[site], dtype=dtype)
                elif isinstance(parity_operator, SiteOperator):
                    parity = parity_operator
                else:
                    parity = dict(parity_operator)[site]
                _validate_site_operator(
                    parity,
                    self.site_legs[site],
                    label="add_fermionic_string parity_operator",
                )
                parity_cache[site] = parity
            return parity_cache[site]

        grouped = {}
        for site, operator in normalized:
            for string_site in range(site):
                grouped.setdefault(string_site, []).append(parity_for(string_site))
            grouped.setdefault(site, []).append(operator)

        final_operators = []
        for site in sorted(grouped):
            factors = grouped[site]
            operator = factors[0] if len(factors) == 1 else _compose_site_operators(*factors)
            dense = operator.as_dense()
            if tol > 0.0:
                is_zero = np.linalg.norm(dense.ravel()) <= float(tol)
            else:
                is_zero = not np.any(dense != 0)
            if is_zero:
                return self
            final_operators.append((site, operator))

        if not final_operators:
            final_operators = [(0, identity_operator(self.site_legs[0], dtype=dtype))]
        return self.add_term(*final_operators, coeff=coeff)
    ...
    def build(self):
        if any(isinstance(term, _ReducedBilinearTerm) for term in self._terms):
            return self._build_uncompressed()
        return self._build_product_trie()

    def _build_product_trie(self):
        """
        Build pure SiteOperator product terms using a shared-prefix channel graph.
        """
        for term in self._terms:
            if not isinstance(term, _ProductTerm):
                raise TypeError(f"Unsupported AutoMPO term type {type(term).__name__}.")

        root = 0
        node_last_site = {root: -1}
        prefix_children = {}
        edge_accum = {}
        next_node = 1
        ...
        for term in self._terms:
            current = root
            items = tuple(zip(term.sites, term.operators))
            for idx, (site, operator) in enumerate(items):
                op_key = _site_operator_signature(operator)
                is_last = idx == len(items) - 1
                if is_last:
                    key = (current, None, site, op_key)
                    if key in edge_accum:
                        edge = edge_accum[key]
                        edge_accum[key] = _TrieEdge(
                            source=edge.source,
                            target=edge.target,
                            site=edge.site,
                            operator=edge.operator,
                            coeff=edge.coeff + term.coeff,
                        )
                    else:
                        edge_accum[key] = _TrieEdge(
                            source=current,
                ...
                child_key = (current, site, op_key)
                child = prefix_children.get(child_key)
                if child is None:
                    child = next_node
                    next_node += 1
                    prefix_children[child_key] = child
                    node_last_site[child] = int(site)
                    edge_accum[(current, child, site, op_key)] = _TrieEdge(
                        source=current,
                        target=child,
                        site=site,
                        operator=operator,
        ...
        internal_nodes = tuple(range(1, next_node))
        left_edge = root
        right_edge = next_node
        dim = right_edge + 1
        if dim >= len(self._terms) + 2:
            return self._build_uncompressed()
        edges = tuple(edge_accum.values())
        edges_by_site = {}
        outgoing_sites = {node: [] for node in range(next_node)}
        for edge in edges:
            target = right_edge if edge.target < 0 else edge.target
            edges_by_site.setdefault(edge.site, []).append(
                _TrieEdge(
                    source=edge.source,
                    target=target,
                    site=edge.site,
            ...
            outgoing_sites.setdefault(edge.source, []).append(edge.site)

        dtype = np.result_type(
            *[operator.dtype for term in self._terms for operator in term.operators],
        ...
        for site, phys_leg in enumerate(self.site_legs):
            ...
            for node in internal_nodes:
                if node_last_site[node] < site and any(
                    next_site > site for next_site in outgoing_sites.get(node, ())
                ):
                    _accumulate_site_operator(
                        visible_blocks,
                        ident,
            ...
            for edge in edges_by_site.get(site, ()):
                _accumulate_site_operator(
                    visible_blocks,
                    edge.operator,
                    edge.source,
