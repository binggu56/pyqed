...
def merge_mps_sites(A, B):
    """
    Merge neighboring non-Abelian MPS sites with standard ``(L, R, P)`` layout.

    The returned two-site tensor has layout ``(L, P_left, R, P_right)``.
    """
...
def split_legs(tensor, axis):
    ...
    split_basis_maps = (tensor.metadata or {}).get("split_basis_maps", {})
    axis_basis_maps = split_basis_maps.get(axis, {})
    ...
    for key, block in tensor.data.items():
        ...
        for entry in layout_entries:
            ...
            elif fusion_leg.pipe.coupling in {"cg", "left", "right"}:
...
def recouple_fused_leg(tensor, axis, target_scheme):
    ...
    for key, block in tensor.data.items():
        ...
        bond_space_source = reduced_bond_space(child_combo, fused_sector, scheme=source_scheme)
        bond_space_target = reduced_bond_space(child_combo, fused_sector, scheme=target_scheme)
