...
def sweep_once(
    ...
    *,
    ...
    if require_cpp_owned_sweep and not owned_sweep:
        detail = (
            f"readiness code {owned_half_sweep_readiness_code}"
            if owned_sweep_candidate
            else "the requested sweep options are incompatible"
        )
        raise RuntimeError(
            "The native C++ owned half-sweep is required, but it is not "
            f"available ({detail})."
        )
    if owned_sweep:
    ...
    for sweep_idx in range(int(nsweeps)):
        ...
        try:
            sweep_result = sweep_once(
                ...
                require_cpp_owned_sweep=require_cpp_owned_sweeps,
                bond_cursor=(
                    None
