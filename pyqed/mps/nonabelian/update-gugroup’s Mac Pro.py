...
from .decompose import state_averaged_svd_two_site, svd_coupled_two_site, svd_two_site
...
def _sector_dim_from_data(tensor, axis):
    ...
    for sector in tensor.qns[axis]:
        if sector in dims:
            continue
        if axis == 1 and tensor.metadata.get("physical_basis") == "reduced_spatial":
            dims[sector] = 1
            continue
...
def two_site_update(
    ...
    *,
    ...
    timing_merge = time.perf_counter() - t0
    merged_dim = int(sum(np.asarray(block).size for block in merged.data.values()))
    merged_blocks = int(len(merged.data))
    ...
    if solver is not None:
        ...
        timing_solve = time.perf_counter() - t0
        ...
        timing_operator = time.perf_counter() - t0
        t0 = time.perf_counter()
    ...
    if optimized_roots is not None:
        ...
        t0 = time.perf_counter()
        left, right, singular_values, trunc_err, kept = state_averaged_svd_two_site(
            optimized_roots,
        ...
        timing_svd = time.perf_counter() - t0
        local_objective["state_averaged_svd"] = True
    else:
        t0 = time.perf_counter()
        coupled_source = (
            optimized.metadata.get("_coupled_physical_source")
            if isinstance(optimized, NonabelianTensor)
            else None
        )
        if coupled_source is not None:
            left, right, singular_values, trunc_err, kept = svd_coupled_two_site(
                coupled_source,
                optimized,
                bond_coupling=bond_coupling,
                max_bond=max_bond,
            ...
            if isinstance(local_objective, dict):
                local_objective["coupled_physical_svd"] = True
        else:
    ...
    if isinstance(local_objective, dict):
        ...
        local_objective.setdefault("merged_dim", merged_dim)
        local_objective.setdefault("merged_blocks", merged_blocks)
        local_objective.setdefault("kept_states", int(kept_states))
        local_objective.setdefault("timing_merge_s", float(timing_merge))
        local_objective.setdefault("timing_operator_s", float(timing_operator))
        local_objective.setdefault("timing_solve_s", float(timing_solve))
        local_objective.setdefault("timing_svd_s", float(timing_svd))
        local_objective.setdefault("timing_total_s", float(time.perf_counter() - t_total))
    if local_guess_used:
        local_objective = dict(local_objective)
