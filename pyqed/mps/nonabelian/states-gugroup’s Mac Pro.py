...
def build_product_spatial_mps(
    ...
    *,
    ...
    When ``reduced_physical=True``, each local charge-spin sector has physical
    degeneracy dimension one. Spin-projection labels ``"up"`` and ``"down"``
    both initialize the spin-adapted singly occupied sector; use
    :func:`build_product_state` when an explicit spin-projection product state
    is required.
    ...
    state_index_by_label = {
        "empty": 0,
        "up": 0,
        "down": 0 if reduced_physical else 1,
        "double": 0,
        "full": 0,
    }
    phys_dims = {
        sector: 1 if reduced_physical else len(indices)
    ...
    for site_index, (label, q_phys, q_left_path, q_right_path) in enumerate(parsed):
        ...
        tensors.append(
            NonabelianTensor(
                ...
                dirs=[-1, 1, 1],
                metadata={"physical_basis": "reduced_spatial"} if reduced_physical else {},
            )
        )
...
def build_reduced_product_spatial_mps(labels, **kwargs):
    """
    Build a TensorKit-style degeneracy-only spatial product MPS.
    """
    kwargs["reduced_physical"] = True
    return build_product_spatial_mps(labels, **kwargs)
...
def build_random_spatial_mps(
    ...
    *,
    ...
    reduced_physical
        If true, use one degeneracy coordinate per local charge-spin sector
        and leave SU(2) representation dimensions to CG/fusion maps.
    seed
...
def build_random_reduced_spatial_mps(nsites, **kwargs):
    """
    Build a random TensorKit-style degeneracy-only spatial MPS.
    """
    kwargs["reduced_physical"] = True
    return build_random_spatial_mps(nsites, **kwargs)
    bond_multiplicity = int(bond_multiplicity)
    if bond_multiplicity < 1:
        raise ValueError("bond_multiplicity must be a positive integer.")
