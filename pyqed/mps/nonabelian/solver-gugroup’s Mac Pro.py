...
def _build_component_sparse_orthonormalized_local_problem(
    ...
    *,
    ...
    # Compact norm metadata is self-contained and exact; its use must not
    # depend on which Hamiltonian carrier happens to own the local action.
    # Tying the metric path to the qchem Hamiltonian provider made a native
    # P/Q carrier fall back to probing every parent-basis vector in Python.
    ...
    prefer_direct_factorized = bool(
        ...
        and (
            ...
            or get_su2_kernel_policy().get("actual") == "cpp"
        )
    )
...
def _solve_preorthonormalized_local_problem(
    ...
    *,
    ...
    try:
        t0 = time.perf_counter() if profile else None
        native_davidson = getattr(
            getattr(problem, "block_table", None),
            "native_davidson",
            None,
        )
        native_davidson_table = getattr(
            getattr(problem, "block_table", None),
            "_native_davidson_table",
            None,
        )
        if (
            ...
            projector_basis is None
            and int(nroots) == 1
            and callable(native_davidson)
            and native_davidson_table is not None
        ):
            native_diag = (
                np.zeros(int(problem.orthonormal_dim), dtype=complex)
                if problem.diag is None
                else np.asarray(problem.diag, dtype=complex)
            )
            native_guess = np.asarray(guess_y[:, 0], dtype=complex)
            native_result = native_davidson(
                native_diag,
                native_guess,
            ...
            if native_result is None or not bool(native_result.get("accepted", False)):
                raise RuntimeError("Native SU(2) block Davidson did not converge.")
            if (
                not bool(native_result.get("converged", False))
                and not bool(allow_unconverged)
            ):
                raise RuntimeError(
                    "Native SU(2) block Davidson reached its iteration limit."
                )
            energies = np.asarray([native_result["energy"]], dtype=float)
            vecs = np.asarray(native_result["vector"], dtype=complex).reshape(-1, 1)
            info = {
                "iterations": int(native_result.get("iterations", 0)),
                "converged": bool(native_result.get("converged", False)),
                "subspace_dim": int(native_result.get("basis_size", 0)),
                "restarts": int(native_result.get("restarts", 0)),
                "native_davidson": True,
                "native_davidson_kind": native_result.get("kind"),
                "native_workspace_reused": bool(
                    native_result.get("workspace_reused", False)
                ),
            }
            native_davidson_used = True
    ...
    info_out = {
        ...
        "native_davidson": bool(
            native_davidson_used or info.get("native_davidson", False)
        ),
        "native_davidson_kind": info.get("native_davidson_kind"),
        "native_workspace_reused": bool(
            info.get("native_workspace_reused", False)
        ),
    ...
    *,
    ...
    if preorthonormalized_problem is not None:
        ...
        return optimized, {
            ...
            "native_davidson": bool(
                solver_info.get("native_davidson", False)
            ),
            "native_davidson_kind": solver_info.get(
                "native_davidson_kind"
            ),
            "native_workspace_reused": bool(
                solver_info.get("native_workspace_reused", False)
            ),
