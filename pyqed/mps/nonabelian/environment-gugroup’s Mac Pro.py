...
from .coupling import clebsch_gordan, ordered_two_m_values, reduced_bond_space
...
_IDENTITY_TWO_SITE_KERNEL_PATH = _cached_einsum_path(
    "al,po,qr,bc->apbqlocr",
    (2, 2),
    (2, 2),
    (2, 2),
    (2, 2),
)
_FACTORIZED_PACKED_LOCAL_DIM = 512
...
def _left_reduced_recoupling_coeff(
    ...
    """
    Project one degeneracy-only left-environment update through hidden CG legs.

    The stored environment blocks carry component coefficients.  For reduced
    physical legs the site tensor omits explicit spin projections, so the
    coefficient multiplying the degeneracy contraction is the projection of
    ``E^{left_irrep} x T^{op_rank}`` onto the outgoing
    ``right_irrep`` component after expanding both site tensors with CG maps.
    """
            ...
            for right_idx, right_irrep in enumerate(W.right_channel_irreps):
                ...
                for row, two_m_left in enumerate(ordered_two_m_values(left_irrep)):
                    for col, two_m_right in enumerate(ordered_two_m_values(right_irrep)):
                        ...
                        if virtual_cg == 0:
                            continue
                        coeff = _left_reduced_recoupling_coeff(
                            q_lb,
                            q_lk,
                            q_pb,
                            q_pk,
            ...
            for right_idx, right_irrep in enumerate(W.right_channel_irreps):
                ...
                for row, two_m_left in enumerate(ordered_two_m_values(left_irrep)):
                    for col, two_m_right in enumerate(ordered_two_m_values(right_irrep)):
                        ...
                        if virtual_cg == 0:
                            continue
                        coeff = _right_reduced_recoupling_coeff(
                            q_lb,
                            q_lk,
                            q_pb,
                            q_pk,
...
def _contract_from_left_blocks_rank_coupled(W, A, E_map, B):
    ...
    reduced_physical = _rank_coupled_degeneracy_only_physical(W)
    a_blocks_by_left = {}
    for (q_lb, q_pb, q_rb), A_block in A.data.items():
                ...
                for q_rb, F_block in right_blocks_by_ket.get(q_rk, ()):
                    for q_p2b, W2_slice in w2_blocks_by_in.get(q_p2k, ()):
                        ...
                        if kernel is None:
                            ...
                            # tmp: (left_out, left_in, mpo_mid, p1_out, p1_in)
                            tmp = np.tensordot(
                                tmp,
                                np.asarray(W2_slice),
                                axes=([2], [0]),
                            )
                            # tmp: (left_out, left_in, p1_out, p1_in, mpo_right, p2_out, p2_in)
                            tmp = np.tensordot(
                                tmp,
                                np.asarray(F_block),
                                axes=([4], [0]),
                            )
                            # tmp: (left_out, left_in, p1_out, p1_in, p2_out, p2_in, right_out, right_in)
                            kernel = np.transpose(tmp, (0, 2, 4, 6, 1, 3, 5, 7)).reshape(
...
def _flat_channel_components(channel_irreps):
    components = []
    for irrep in channel_irreps:
        for two_m in ordered_two_m_values(irrep):
            components.append((irrep, int(two_m)))
    return tuple(components)


@lru_cache(maxsize=None)
def _pair_channel_basis(child_left, child_right, fused_sector, slot):
    space = reduced_bond_space((child_left, child_right), fused_sector, scheme="left")
    ...
    raise ValueError(
        f"Missing pair-coupling slot {slot} for {(child_left, child_right)!r} -> {fused_sector!r}."
    )
...
@lru_cache(maxsize=None)
def _coupled_two_site_recoupling_coeff(
    q_lb,
    q_lk,
    q_p1b,
    q_p1k,
    ...
    """
    Hidden-m projection for a reduced coupled two-site local operator block.

    The coupled local basis stores only degeneracy channels.  This coefficient
    expands the left environment, two local MPO factors, right environment, and
    physical-pair fusion channels through their CG maps, then projects back
    onto the outgoing coupled channel.
    """
    left_env = _component_basis_matrix(q_lb, q_lk, left_irrep, int(two_m_left))
    right_env = _component_basis_matrix(q_rb, q_rk, right_irrep, int(two_m_right))
    if not np.any(left_env) or not np.any(right_env):
        return 0.0

    w1_component = _rank_coupled_component(
        left_irrep,
        mid_irrep,
        int(two_m_left),
        int(two_m_mid),
    )
    w2_component = _rank_coupled_component(
        mid_irrep,
        right_irrep,
        int(two_m_mid),
        int(two_m_right),
    ...
    spinor = SU2Irrep(1)
    w1_phys = _physical_component_matrix(
        q_p1b,
        q_p1k,
        SU2Irrep(0) if w1_diagonal else spinor,
        0 if w1_diagonal else w1_component,
        bool(w1_diagonal),
    )
    w2_phys = _physical_component_matrix(
        q_p2b,
        q_p2k,
        SU2Irrep(0) if w2_diagonal else spinor,
        0 if w2_diagonal else w2_component,
        bool(w2_diagonal),
    )
    if not np.any(w1_phys) or not np.any(w2_phys):
        return 0.0

    out_pair = _pair_channel_basis(q_p1b, q_p2b, q_fb, int(out_slot))
    in_pair = _pair_channel_basis(q_p1k, q_p2k, q_fk, int(in_slot))

    q_lb_irrep = _sector_irrep(q_lb)
    q_lk_irrep = _sector_irrep(q_lk)
    q_p1b_irrep = _sector_irrep(q_p1b)
    q_p1k_irrep = _sector_irrep(q_p1k)
    q_p2b_irrep = _sector_irrep(q_p2b)
    q_p2k_irrep = _sector_irrep(q_p2k)
    q_fb_irrep = _sector_irrep(q_fb)
    q_fk_irrep = _sector_irrep(q_fk)
    q_rb_irrep = _sector_irrep(q_rb)
    q_rk_irrep = _sector_irrep(q_rk)

    coeff = 0.0
    lb_ms = ordered_two_m_values(q_lb_irrep)
    lk_ms = ordered_two_m_values(q_lk_irrep)
    p1b_ms = ordered_two_m_values(q_p1b_irrep)
    p1k_ms = ordered_two_m_values(q_p1k_irrep)
    p2b_ms = ordered_two_m_values(q_p2b_irrep)
    p2k_ms = ordered_two_m_values(q_p2k_irrep)
    fb_ms = ordered_two_m_values(q_fb_irrep)
    fk_ms = ordered_two_m_values(q_fk_irrep)
    rb_ms = ordered_two_m_values(q_rb_irrep)
    rk_ms = ordered_two_m_values(q_rk_irrep)

    for i_lb, m_lb in enumerate(lb_ms):
        for i_lk, m_lk in enumerate(lk_ms):
            env_l = left_env[i_lb, i_lk]
            if env_l == 0:
                continue
            for i_p1b, m_p1b in enumerate(p1b_ms):
                for i_p1k, m_p1k in enumerate(p1k_ms):
                    op1 = w1_phys[i_p1b, i_p1k]
                    if op1 == 0:
                        continue
                    for i_p2b, m_p2b in enumerate(p2b_ms):
                        row_out_pair = i_p1b * len(p2b_ms) + i_p2b
                        for i_p2k, m_p2k in enumerate(p2k_ms):
                            op2 = w2_phys[i_p2b, i_p2k]
                            if op2 == 0:
                                continue
                            row_in_pair = i_p1k * len(p2k_ms) + i_p2k
                            for i_fb, m_fb in enumerate(fb_ms):
                                pair_out = out_pair[row_out_pair, i_fb]
                                if pair_out == 0:
                                    continue
                                for i_fk, m_fk in enumerate(fk_ms):
                                    pair_in = in_pair[row_in_pair, i_fk]
                                    if pair_in == 0:
                                        continue
                                    for i_rb, m_rb in enumerate(rb_ms):
                                        cg_out = clebsch_gordan(
                                            q_lb_irrep,
                                            q_fb_irrep,
                                            q_rb_irrep,
                                            m_lb,
                                        ...
                                        if cg_out == 0:
                                            continue
                                        for i_rk, m_rk in enumerate(rk_ms):
                                            env_r = right_env[i_rb, i_rk]
                                            if env_r == 0:
                                                continue
                                            cg_in = clebsch_gordan(
                                                q_lk_irrep,
                                                q_fk_irrep,
                                                q_rk_irrep,
                                                m_lk,
...
def _build_reduced_rank_coupled_coupled_matrix(
    E_map,
    W1,
    W2,
    F_map,
    ...
    if not (
        isinstance(W1, RankCoupledMPO)
        and isinstance(W2, RankCoupledMPO)
        and _rank_coupled_degeneracy_only_physical(W1)
        and _rank_coupled_degeneracy_only_physical(W2)
    ):
        return None
    if coupled_template.rank != 3 or coupled_template.fusion_legs[1] is None:
        return None
    pipe = coupled_template.fusion_legs[1].pipe
    if pipe is None:
        return None

    left_components = _flat_channel_components(W1.left_channel_irreps)
    mid_components = _flat_channel_components(W1.right_channel_irreps)
    w2_left_components = _flat_channel_components(W2.left_channel_irreps)
    right_components = _flat_channel_components(W2.right_channel_irreps)
    if mid_components != w2_left_components:
        return None

    total_dim = sum(entry.size for entry in coupled_layout)
    dtype = np.result_type(_mpo_dtype(W1), _mpo_dtype(W2), complex)
    matrix = np.zeros((total_dim, total_dim), dtype=dtype)

    for in_entry in coupled_layout:
        q_lk, q_fk, q_rk = in_entry.key
        in_pipe_entries = tuple(pipe.entries_for_sector(q_fk))
        for out_entry in coupled_layout:
            q_lb, q_fb, q_rb = out_entry.key
            E_block = E_map.get((q_lb, q_lk))
            F_block = F_map.get((q_rb, q_rk))
            ...
            out_pipe_entries = tuple(pipe.entries_for_sector(q_fb))
            for in_pipe in in_pipe_entries:
                if int(in_pipe.local_dim) != 1:
                    continue
                q_p1k, q_p2k = in_pipe.child_sectors
                in_local_offset = in_entry.offset + int(in_pipe.offset) * in_entry.shape[2]
                for out_pipe in out_pipe_entries:
                    if int(out_pipe.local_dim) != 1:
                        continue
                    q_p1b, q_p2b = out_pipe.child_sectors
                    W1_block = W1.block(q_p1b, q_p1k)
                    W2_block = W2.block(q_p2b, q_p2k)
                    if W1_block is None or W2_block is None:
                        continue
                    W1_arr = np.asarray(W1_block)
                    W2_arr = np.asarray(W2_block)
                    out_local_offset = out_entry.offset + int(out_pipe.offset) * out_entry.shape[2]
                    sub = np.zeros(
                        (
                            out_entry.shape[0] * out_entry.shape[2],
                            in_entry.shape[0] * in_entry.shape[2],
                        ),
                        dtype=dtype,
                    )
                    w1_diagonal = q_p1b == q_p1k
                    w2_diagonal = q_p2b == q_p2k
                    for x, (left_irrep, two_m_left) in enumerate(left_components):
                        if x >= E_arr.shape[0] or x >= W1_arr.shape[0]:
                            continue
                        E_x = E_arr[x]
                        if not np.any(E_x):
                            continue
                        for y, (mid_irrep, two_m_mid) in enumerate(mid_components):
                            if y >= W1_arr.shape[1] or y >= W2_arr.shape[0]:
                                continue
                            w1_scalar = W1_arr[x, y, 0, 0]
                            if w1_scalar == 0:
                                continue
                            for z, (right_irrep, two_m_right) in enumerate(right_components):
                                if z >= W2_arr.shape[1] or z >= F_arr.shape[0]:
                                    continue
                                w2_scalar = W2_arr[y, z, 0, 0]
                                if w2_scalar == 0:
                                    continue
                                recoupled = _coupled_two_site_recoupling_coeff(
                                    q_lb,
                                    q_lk,
                                    q_p1b,
                                    q_p1k,
                                ...
                                if recoupled == 0:
                                    continue
                                factor = np.asarray(w1_scalar * w2_scalar * recoupled, dtype=dtype)
                                sub += factor * np.einsum(
                                    "ab,cd->acbd",
                                    E_x,
                                    F_arr[z],
                                    optimize=True,
                    ...
                    if not np.any(sub):
                        continue
                    for out_l in range(out_entry.shape[0]):
                        for out_r in range(out_entry.shape[2]):
                            row = out_local_offset + out_l * out_entry.shape[1] * out_entry.shape[2] + out_r
                            sub_row = out_l * out_entry.shape[2] + out_r
                            for in_l in range(in_entry.shape[0]):
                                for in_r in range(in_entry.shape[2]):
                                    col = in_local_offset + in_l * in_entry.shape[1] * in_entry.shape[2] + in_r
                                    sub_col = in_l * in_entry.shape[2] + in_r
                                    matrix[row, col] += sub[sub_row, sub_col]
    return matrix
...
def _build_block_sparse_local_actions(
    ...
    if dim > _FACTORIZED_PACKED_LOCAL_DIM:
        _out_entries, factorized_terms = _precompute_two_site_block_env_factorized_terms(
            E_map,
            W1,
            W2,
            F_map,
        ...
        packed_apply.out_entries = _out_entries
        tensor_apply = None
        reduced_apply = None
        identity_like = False
    else:
    ...
    def _bond_operator_from_envs(self, bond, two_site_template, *, left_env, right_env):
        ...
        if (
            ...
            def coupled_matrix_factory(
                ...
                return _build_reduced_rank_coupled_coupled_matrix(
                    left_env,
                    W1,
                    W2,
                    right_env,
        ...
        if tensor_matvec is None:
            return LocalOperator(
                packed_matvec=packed_matvec,
                packed_block_matrices=getattr(packed_matvec, "block_matrices", None),
                coupled_matrix_factory=coupled_matrix_factory,
                diag=diag,
        ...
        if self.direction == "lr":
            self.current_env = (
                ...
                if self.chain.rank_coupled
                    ...
                    self.current_env,
                    left_site,
                    self.chain.site_layouts[bond]["sector_slices"][1],
                )
            )
        else:
            self.current_env = (
                ...
                if self.chain.rank_coupled
                    ...
                    self.current_env,
                    right_site,
                    self.chain.site_layouts[bond + 1]["sector_slices"][1],
                )
            )
        return self
