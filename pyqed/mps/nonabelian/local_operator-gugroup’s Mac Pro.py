...
def _factorized_block_two_step_matrices(left_stack, right_stack):
    ...
    :param left_stack: Batched left factors with shape ``tlkwab``.
    :param right_stack: Batched right factors with shape ``twqrdc``.
    :returns: ``(left_matrix, right_matrix, shape_info)``.
    """
...
def _apply_factorized_block_two_step(left_stack, block_in, right_stack, *, matrices=None):
    ...
    This is equivalent to::
        ...
        tmp = einsum("tlkwab,kbcr->tlwacr", left_stack, block_in)
        out = einsum("tlwacr,twqrdc->ladq", tmp, right_stack)

    but reshapes the two contractions into matrix products, which is much
    faster for the larger SU(2) local blocks encountered in qchem DMRG.
    """
