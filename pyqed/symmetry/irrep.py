...
def _restore_leg(sectors, dims, symmetry):
    return Leg(sectors, dims, symmetry=symmetry)


@dataclass(frozen=True, init=False, eq=False)
class Leg:
    """Unoriented tensor-index space with optional symmetry sectors.
    ...
    def __init__(self, sectors, dims=None, *, symmetry=None):
    ...
    def __eq__(self, other):
        if not isinstance(other, Leg) or self.symmetry != other.symmetry:
            return False
    ...
    def __reduce__(self):
        return _restore_leg, (self.sectors, dict(self.dims), self.symmetry)
    ...
    @classmethod
    def from_dims(cls, sector_dims, sectors=None, *, symmetry=None):
        if sectors is None:
            sectors = tuple(sector_dims)
        return cls(tuple(sectors), sector_dims, symmetry=symmetry)

    @classmethod
    def from_slices(cls, sector_slices, *, symmetry=None):
        return cls(
            tuple(sector_slices),
            {
    ...
    def dual(self):
        if self.symmetry is None:
            raise TypeError("dual requires a symmetry-aware Leg.")
        dims = {}
        for irrep in self.irreps:
            dual = Irrep(self.symmetry.dual(irrep.charge), irrep.alpha)
            dims[dual] = dims.get(dual, 0) + self.dims[irrep]
        return Leg(dims, symmetry=self.symmetry)
...
class IrrepTensor:
    """Block matrix between two symmetry-aware :class:`Leg` spaces."""
    ...
    @classmethod
    def zeros(cls, bra: Leg, ket: Leg, op: OpIrrep) -> "IrrepTensor":
    ...
    def from_site_operator(
        ...
        *,
        ...
        Operators containing multiple charge transfers, such as ``Sx`` in a
        ``2Sz`` basis, must be split into homogeneous components first.
        """
        derived_site = site.leg
        _symmetry, irreps, state_indices = _site_irrep_layout(site)
        if leg is None:
            leg = derived_site
        elif leg != derived_site:
        ...
        if site.charges is None:
            ...
            for irrep, indices in zip(irreps, state_indices):
                charge = (
                    tuple(int(value) for value in irrep.charge)
                    if isinstance(irrep.charge, tuple)
                    else (int(irrep.charge),)
                )
                for index in indices:
                    state_charges[index] = charge
        ...
        for row, column in zip(rows, columns):
            out_charge = state_charges[int(row)]
            in_charge = state_charges[int(column)]
            transfer = tuple(
                int(out_value) - int(in_value)
                for out_value, in_value in zip(out_charge, in_charge)
            )
        ...
        if op_charge is None:
            ...
            op_charge = transfer[0] if len(transfer) == 1 else transfer
        else:
            if isinstance(op_charge, Iterable) and not isinstance(op_charge, (str, bytes)):
                normalized = tuple(int(value) for value in op_charge)
            else:
                normalized = (int(op_charge),)
        ...
        for bra_irrep, bra_indices in zip(leg.irreps, state_indices):
            for ket_irrep, ket_indices in zip(leg.irreps, state_indices):
                if not leg.symmetry.allows(
                    ...
                    continue
                block = np.asarray(matrix)[np.ix_(bra_indices, ket_indices)]
                if np.any(np.abs(block) > float(atol)):
                    blocks[(bra_irrep, ket_irrep)] = block.copy()
        return cls(leg, leg, OpIrrep(op_charge), blocks)

    @classmethod
    def identity(cls, site: Leg) -> "IrrepTensor":
        blocks = {
