...
1020  format('Total Energy =', f10.5/ , & 
      ...
      call variation(ndim,ntraj,w,x,xVar) 

      print *,'Dispersion of atom 1 = ',xVar
      print *,'Dispersion/Interparticle Distance = ', xVar/(R0/2d0)   
...
! ----------------------------------------------------------------
      subroutine variation(ndim,ntraj,w,x,xVar) 
      implicit real*8(a-h,o-z) 

      real*8, dimension(ntraj) :: w 
      real*8, dimension(ndim,ntraj) :: x 
      ...
      do i=1,ntraj 
        xAve = xAve + x(1,i)*w(i) 
        xSqdAve = xSqdAve + x(1,i)**2*w(i) 
      enddo 
      
      xVar = dsqrt(xSqdAve - xAve**2) 
      end subroutine 
