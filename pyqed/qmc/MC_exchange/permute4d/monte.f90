...
!	5. number of nearest-neighbours 
...
! --- compute and print energy
    ...
    hError = dsqrt(hVar) / dsqrt(dble(NWalkers) * dble(MCSteps))
    
    ! total error for the ground-state energy


    GSEError = hError/(1d0 + dble(NN)*pAve) + hAve * dble(NN) * pError / (1d0 + dble(NN)*pAve)**2   
...
! --- potential energy     
    ...
    xcm = (x(1)+x(2)+x(3)+x(4))/4d0
 
    env = akt*xcm**2/2d0 + akv*((abs(x(1)-x(2))-R0)**2/2d0 + & 
      (abs(x(1)-x(3))-R0)**2/2d0 + (abs(x(2)-x(3))-R0)**2/2d0  + & 
      (abs(x(4)-x(1))-R0)**2/2d0 + (abs(x(4)-x(2))-R0)**2/2d0 + & 
      (abs(x(4)-x(3))-R0)**2/2d0 ) - R0**2/2d0
...
subroutine MetropolisStep(x)
    ...
    hSqdSum = hSqdSum + ( (enk+env)*(1d0+ dble(NN) * p) + dble(NN) * rDp*p )**2
