...
! --- compute and print energy
    ...
    hSum = enkSum + enkpSum + envSum + envpSum + rDpSum
    ...
    GSEError = hError/(1d0+pAve) + hAve*pError/(1d0+pAve)**2   
...
1012 format('Permutation overlap = ',f14.8, ' +/- ', f14.7//,   & 
    ...
    write(6, 1013) envSym, envSym-envAve, enkSym, enkSym-enkAve, enkSym+envSym, GSEError  
...
subroutine MetropolisStep(x)
    ...
    hSqdSum = hSqdSum + ( (enk+env)*(1d0+p) + rDp )**2  
